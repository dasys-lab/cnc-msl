...
inline int CMD_Grabber_Info_GET(unsigned int *lArm, unsigned int *rArm, bool *lHandicap, bool *rHandicap)
{
    ...
    *rArm = gInfo.rightArm;
    if (lHandicap != NULL)
        *lHandicap = gInfo.leftHandicapped;
    if (rHandicap != NULL)
        *rHandicap = gInfo.rightHandicapped;
    return lt;
}
...
inline void CMD_Kicker_SET(unsigned char power, unsigned char highThreshold = ENGAGED_THRESHOLD_HIGH, unsigned char lowThreshold = ENGAGED_THRESHOLD_LOW)
{
    CMD_Kicker kicker;
    kicker.power = power;
...
inline int CMD_Info_GET(unsigned short *Voltage_logic, unsigned short *Voltage_power12, unsigned short *Voltage_power24, unsigned short *Kicker_voltage,
                        bool *justKicked)
{
    CMD_Info info;
    ...
    if (Voltage_logic != NULL)
        *Voltage_logic = info.Voltage_logic;
    if (Voltage_power12 != NULL)
        *Voltage_power12 = info.Voltage_power12;
    if (Voltage_power24 != NULL)
        *Voltage_power24 = info.Voltage_power24;
    if (Kicker_voltage != NULL)
        *Kicker_voltage = info.Kicker_voltage;
    if (justKicked != NULL)
        *justKicked = info.justKicked;

    return lf;
}

inline void HWcomm_Set_RobotStatus(unsigned short Voltage_logic, unsigned short Voltage_power12, unsigned short Voltage_power24, unsigned short Kicker_voltage,
                                   bool justKicked)
{
    CMD_Info info;
