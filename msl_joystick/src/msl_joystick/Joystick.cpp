...
namespace msl_joystick
{
            ...
            rqt_gui_cpp::Plugin(), uiWidget(0), sendMsgTimer(0)
    {
        ...
        joySub = rosNode->subscribe("/joy", 10, &Joystick::onJoyMsg, (Joystick *)this);
...
// QT GUI Signal and Slots Methods
    ...
    void Joystick::onJoyMsg(sensor_msgs::JoyPtr msg)
    {
        ...
        // robotid
        joycmd.robotId = this->robotId;

        // ballHandle stuff
        joycmd.ballHandleLeftMotor = this->ballHandleLeftMotor;
        joycmd.ballHandleRightMotor = this->ballHandleRightMotor;
        if (this->useBallHandle)
        {
            joycmd.ballHandleState = msl_msgs::JoystickCommand::BALL_HANDLE_ON;
        }
        else
        {
            joycmd.ballHandleState = msl_msgs::JoystickCommand::BALL_HANDLE_OFF;
        }

        // kicker stuff
        joycmd.kickPower = this->kickPower;
        ...
        if (abs(msg->axes.at(0)) == 0.0 || abs(msg->axes.at(1)) == 0.0)
        {
            ...
            joycmd.motion.rotation = std::numeric_limits<double>::quiet_NaN();
            joycmd.motion.angle = std::numeric_limits<double>::quiet_NaN();
        }
        else
        {
...
//            if (keyPressed[0] == true && keyPressed[2] == true)
//            {
//                joycmd.motion.angle = (1.25 * M_PI);
//            }
//            else if (keyPressed[0] == true && keyPressed[3] == true)
//            {
//                joycmd.motion.angle = (0.75 * M_PI);
//            }
//            else if (keyPressed[1] == true && keyPressed[2] == true)
//            {
//                joycmd.motion.angle = (1.75 * M_PI);
//            }
//            else if (keyPressed[1] == true && keyPressed[3] == true)
//            {
//                joycmd.motion.angle = (0.25 * M_PI);
//            }
            if (msg->axes.at(1) >= 0.0)
            {
                joycmd.motion.angle = M_PI;
                joycmd.motion.translation = abs(msg->axes.at(1)) * this->translation;
            }
            else if (msg->axes.at(1) < 0.0)
            {
                joycmd.motion.angle = 0;
                joycmd.motion.translation = abs(msg->axes.at(1)) * this->translation;
            }
            else if (msg->axes.at(0) >= 0.0)
            {
                joycmd.motion.angle = (1.5 * M_PI);
                joycmd.motion.translation = abs(msg->axes.at(0)) * this->translation;
            }
            else if (msg->axes.at(0) < 0.0)
            {
                joycmd.motion.angle = (0.5 * M_PI);
                joycmd.motion.translation = abs(msg->axes.at(0)) * this->translation;
            }
            else
            {
                joycmd.motion.translation = 0;
