...
IMU::IMU(Communication *comm)
    : Worker("IMU")
{
    this->comm = comm;
    this->i2c = new BlackLib::BlackI2C(BlackLib::I2C_2, ADR_G);
    if (!this->i2c->open(BlackLib::ReadWrite))
    {
	std::cerr << "IMU: I2C could not be opened!" << std::endl;
    }
...
void IMU::sendData()
{
    ...
    gettimeofday(&last_sended, NULL);
    msg.time = (unsigned long long)last_sended.tv_sec * 1000000 + last_sended.tv_usec;

    this->comm->onRosIMUData3455796956(msg);
}
