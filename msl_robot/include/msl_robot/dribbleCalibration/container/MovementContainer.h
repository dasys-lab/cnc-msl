...
namespace msl
{
	...
	private:
		...
		shared_ptr<geometry::CNPointAllo> alloAlignPoint;

		shared_ptr<geometry::CNPointEgo> getEgoDestinationPoint(Movement movement, double distance);
		shared_ptr<geometry::CNPointEgo> calcNewAlignPoint(Movement curMove);
		bool checkFieldLines(shared_ptr<geometry::CNPointEgo> egoDest);
