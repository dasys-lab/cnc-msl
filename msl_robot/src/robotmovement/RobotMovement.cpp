...
msl_actuator_msgs::MotionControl RobotMovement::moveToPoint(MovementQuery &query)
{
    ...
    mc.motion.angle = egoTarget->angleZ() - mc.motion.rotation * this->sampleTime; // 1/30 s= time step , time step * omega = phi
 ...
 */
msl_actuator_msgs::MotionControl RobotMovement::placeRobot(geometry::CNPointEgo dest, nonstd::optional<geometry::CNPointEgo> headingPoint)
{
    msl_actuator_msgs::MotionControl mc;
...
msl_actuator_msgs::MotionControl RobotMovement::moveToFreeSpace(MovementQuery &query)
{
    ...
    for (int i = 0; i < 100 && fringe->size() > 0; i++)
    {
        ...
        {
            next->at(j)->val =
                evalPointDynamic(next->at(j)->midP, *teamMatePosition, *ownPos, **wm->obstacles->getRawObstaclesAlloBuffer().getLastValidContent());
 ...
 */
double RobotMovement::evalPointDynamic(geometry::CNPointAllo alloP, geometry::CNPointAllo alloPassee, geometry::CNPositionAllo ownPos,
 ...
 */
std::valarray<double> RobotMovement::ptController(MovementQuery &query, double translation, double rotation)
{
