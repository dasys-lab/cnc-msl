...
// mask   000000FF
// -----------------
// id     XXXXXX60 <-- this is how the id has to look like
prog_char can_filter[] = {
	...
	// mask0 -> buffer RXB0
	// mask1 -> buffer RXB1
	MCP2515_FILTER_EXTENDED(0x000000FF),	// Mask 0
	MCP2515_FILTER_EXTENDED(0x000000FF),	// Mask 1
};
...
void parse_default(tExtendedCAN *m) {
	...
	switch (m->data[0]) {
		...
		//		kicker_add_rotate_job(m->data[1]);
		//	break;
		case CMD_KICK:
			if (timer_get_ms() > 1000) {
				if (m->header.length == 2)
					kicker_add_kick_job(m->data[1]);
				else if (m->header.length == 3)
					kicker_add_kick_job_forced(m->data[1], m->data[2]);
			}
			break;
		//case CMD_SET_PULSE_WIDTH:
		//	if (m->header.length == 7) {
		//		tmp = (m->data[1] << 8) + m->data[2];
		//		kicker_set_servo_pos(1, tmp);
		//		tmp = (m->data[3] << 8) + m->data[4];
		//		kicker_set_servo_pos(2, tmp);
		//		tmp = (m->data[5] << 8) + m->data[6];
		//		kicker_set_servo_pos(3, tmp);
		//	}
		//	break;
		case CMD_SET_MAX_VOLTAGE:
			if (m->header.length == 1) {
				booster_set_max_voltage(m->data[1]);
			}
			break;
...
void parse_manual(tExtendedCAN *m) {
	...
	static uint8_t release_time = 33;
	uint16_t tmp = 0;
	uint8_t i;
	char str[8];

	// set the power of a shot
	// full power is round about 30
	// a slow pass is about 8
	if (m->data[0] == 's') {
	...
	// rotate (send r1 to rotate to kicker 1)
	// this uses the official rotating mechanism
	// (including interlocking)
	//else if (m->data[0] == 'r') {
	//	if (m->data[1] >= 0x31 && m->data[1] <= 0x33) {
	//		kicker_add_rotate_job(m->data[1] - 0x30);
	//	}
	//}
	// this is the forced alternative, but this
	// checks if the lock is released
	// use this if you want to override
	// the switch
	//else if (m->data[0] == 'R') {
	//	if (m->data[1] >= 0x31 && m->data[1] <= 0x33) {
	//		kicker_rotate_servo(m->data[1] - 0x30);
	//	}
	//}
	// toggle the interlocking mechanism
	//else if (m->data[0] == 'l') {
	//	kicker_toggle_interlock();
	//}
