...
#define MAX_BOOST_TIME			10000 // ms
...
struct BOOSTER_STATE {
	uint8_t kicker_pos    : 2; //< the position of the servo
...
struct BOOSTER_INFO {
	...
	uint8_t  capacitors_voltage;	//< the voltage of the capacitors (Volt)
};
...
void booster_send_info(void) {
	...
	info.state.power_state = (IS_SET(POWER) > 0);
	...
	//info.state.kicker_pos = kicker_get_pos();
	info.state.kicker_pos = 0;
	info.supply_voltage = get_supply_raw_voltage();
	info.capacitors_voltage = (uint8_t) get_capacitors_voltage();

	can_put_cmd(CMD_STATE, (uint8_t *)&info, 4);
}
