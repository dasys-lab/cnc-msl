...
unsigned int rtl8019BeginPacketRetreive(void)
{
    ...
    rtl8019Write(CR, 0x22);
    for(i = 0; i <= 20; i++)
        if(rtl8019Read(RTLISR) & 1<<6)
            break;
    rtl8019Write(RTLISR, 1<<6);
...
void rtl8019Overrun(void)
{
	...
	rtl8019Write(CURR, RXSTART_INIT);
	rtl8019Write(CR, 0x22);
	rtl8019Write(RTLISR, 0x10);
	rtl8019Write(TCR, TCR_INIT);
	
    if(resend)
        rtl8019Write(CR, 0x26);

    rtl8019Write(RTLISR, 0xFF);
}
...
void rtl8019Init(void)
{
	...
	// do soft reset
	rtl8019Write( RTLISR, rtl8019Read(RTLISR) ) ;
	for(i=0;i<5;i++) delay_ms(10);
...
{
	unsigned char byte = rtl8019Read(RTLISR);
