...
//	odo_buff_pos = 0;
	...
	controller_stat.motion_out.x = 0;
	controller_stat.motion_out.y = 0;
	controller_stat.motion_out.rotation = 0;
...
//	accel_vector[1] = -(controller_stat.actual_motion_vector.rotation*controller_stat.actual_motion_vector.x)/(64);
	...
	accel_vector[0] = (controller_stat.motion_goal.rotation*controller_stat.motion_goal.y)/ROTATION_PRECISION;
	accel_vector[1] = -(controller_stat.motion_goal.rotation*controller_stat.motion_goal.x)/ROTATION_PRECISION;


	accel_vector[0] *= delta_time;
	accel_vector[1] *= delta_time;
	accel_vector[0] /= TIME_TICKS_PER_SECOND;
	accel_vector[1] /= TIME_TICKS_PER_SECOND;
//DEBUG
//	accel_vector[0] = 0;
//	accel_vector[1] = 0;
//END DEBUG

	//Subtract since force within the robot system is relevant
	accel_vector[0] -= delta_motion[0];
	accel_vector[1] -= delta_motion[1];
	temp_vector[0] = accel_vector[0];
	temp_vector[1] = accel_vector[1];
	if (accel_vector[0] < -FORCE_RANGE || accel_vector[0] > FORCE_RANGE ||
		accel_vector[1] < -FORCE_RANGE || accel_vector[1] > FORCE_RANGE) {
		temp_float = sqrt((float)(accel_vector[0]*accel_vector[0]+accel_vector[1]*accel_vector[1]));	
		accel_vector[0] *= FORCE_RANGE;
		accel_vector[0] /= temp_float;
		accel_vector[1] *= FORCE_RANGE;
		accel_vector[1] /= temp_float;
	}
	cn_get_force_correction(accel_vector, correction_vector);
	delta_motion[0] += correction_vector[0]-(accel_vector[0]-temp_vector[0]);
	delta_motion[1] += correction_vector[1]-(accel_vector[1]-temp_vector[1]);
...
//END DEBUG
	...
	controller_stat.pid_e[0] = controller_stat.motion_goal.x - controller_stat.actual_motion_vector.x;
	controller_stat.pid_e[1] = controller_stat.motion_goal.y - controller_stat.actual_motion_vector.y;
	controller_stat.pid_e[2] = controller_stat.motion_goal.rotation - controller_stat.actual_motion_vector.rotation;
	
	for (i=0; i<2;i++) {
		controller_stat.pid_eD[i] = controller_params_active.kdi * controller_stat.pid_eD[i];
		
		controller_stat.pid_eD[i] += (controller_stat.pid_e[i]-controller_stat.pid_eOld[i]) * controller_params_active.kd;	  //substract new value
		...
		controller_stat.pid_eD[2] = controller_params_active.kdi * controller_stat.pid_eD[i];
		
		controller_stat.pid_eD[2] += (controller_stat.pid_e[2]-controller_stat.pid_eOld[2]) * controller_params_active.rot_err_accel_w;	  //substract new value
		controller_stat.pid_eD[2] /= 8192l;
		controller_stat.pid_eD[2] = CLAMP(controller_stat.pid_eD[2],-8192l*CN_CONTROLLER_MAX_E, 8192l*CN_CONTROLLER_MAX_E);



	controller_stat.motion_out.x = (controller_stat.pid_e[0]*controller_params_active.kp + controller_stat.pid_eI[0]*controller_params_active.ki)/8192l +
									controller_stat.pid_eD[0]; 
	controller_stat.motion_out.y = (controller_stat.pid_e[1]*controller_params_active.kp + controller_stat.pid_eI[1]*controller_params_active.ki)/8192l +
									controller_stat.pid_eD[1]; 
	controller_stat.motion_out.rotation = (controller_stat.pid_e[2]*controller_params_active.rot_err_w + controller_stat.pid_eI[2]*controller_params_active.rot_err_velo_w)/8192l +
									controller_stat.pid_eD[2]; 

	
	controller_stat.pid_eI[0] += controller_stat.pid_e[0];
	controller_stat.pid_eI[1] += controller_stat.pid_e[1];
	controller_stat.pid_eI[2] += controller_stat.pid_e[2];

	controller_stat.pid_eI[0] = CLAMP(controller_stat.pid_eI[0],-controller_params_active.max_error_int,controller_params_active.max_error_int);
	controller_stat.pid_eI[1] = CLAMP(controller_stat.pid_eI[1],-controller_params_active.max_error_int,controller_params_active.max_error_int);
	controller_stat.pid_eI[2] = CLAMP(controller_stat.pid_eI[2],-412160,412160); //16PI*ROTATION_PRECISION
	
	controller_stat.pid_eOld[0] = controller_stat.pid_e[0];
	controller_stat.pid_eOld[1] = controller_stat.pid_e[1];
	controller_stat.pid_eOld[2] = controller_stat.pid_e[2];
...
//TODO: Optimise and reconsider ranges
	//rotation first:
	temp = controller_stat.motion_out.rotation;
	...
	//add trans in x-dir:
	temp = controller_stat.motion_out.x;
	temp *= 600;
...
//	temp *= 8; //64/8  
	...
	//add trans in y-dir:
	temp = controller_stat.motion_out.y;	
	...
	for(i=0; i < 3; i++) {		
		...
		temp = controller_stat.rpm_goal[i];
