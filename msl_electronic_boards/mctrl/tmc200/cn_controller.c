...
cart_motion_vector xhuge odometry_log[15000];
...
void cn_controller_main_control() {
	...
	if(cn_controller_has_error() == 0) {
		...
		// always do last
		controller_direct_pwm_final.pwm[0] = 0x00;
		controller_direct_pwm_final.pwm[1] = 0x00;
		controller_direct_pwm_final.pwm[2] = 0x00;
...
void cn_controller_calc_odometry() {
	...
	delta_time = (now - last_cycle_time);
...
//TODO: do some optimisations:
	...
	if(controller_log_odometry==1) {
		odometry_log[odolog_write_pos].x = controller_stat.actual_motion_vector.x;
		odometry_log[odolog_write_pos].y = controller_stat.actual_motion_vector.y;
		odometry_log[odolog_write_pos].rotation = controller_stat.actual_motion_vector.rotation;
		if (++odolog_write_pos >= 15000) odolog_write_pos = 0;
	}
}
