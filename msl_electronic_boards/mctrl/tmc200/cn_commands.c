...
void cn_command_configure_set_cycle_time(ubyte* milsec) {
	if (*milsec == 0) {
		cn_write_response(GROUP_ERROR_RESPONSE,CMD_OUT_OF_RANGE,milsec,1); 
					...
					//+-------------+ microsec (long!)
//TODO: This should refer to constants in cn_time.h					
		cycle_time = ((*milsec * 1000l) * 10) / 512;
...
void cn_command_configure_command_timeout(ubyte milsec) {
  ...
  unsigned long timeout = ((milsec * 1000l) * 10) / 512;
...
void cn_command_configure_toggle_odometry_log(ubyte* toggle) {
	if (*toggle==0x00) controller_log_odometry = 0;
	else controller_log_odometry = 1;
...
void cn_command_control_set_all_pwm(sword val1, sword val2, sword val3) {
  ...
  if(val1 < 0) { dir1 = DIRECTION_CW; val1 *= -1; } else { dir1 = DIRECTION_CCW; }
  if(val2 < 0) { dir2 = DIRECTION_CW; val2 *= -1; } else { dir2 = DIRECTION_CCW; }
  if(val3 < 0) { dir3 = DIRECTION_CW; val3 *= -1; } else { dir3 = DIRECTION_CCW; }
...
void cn_command_request_path_vector() {
	...
	answer[2] = (sword)controller_stat.actual_motion_vector.rotation;
...
void cn_command_request_odometry_log(uword pos) {
	sword answer[3];
	answer[0] = (sword) odometry_log[pos].x;
	answer[1] = (sword) odometry_log[pos].y;
	answer[2] = (sword) odometry_log[pos].rotation;
	cn_write_response(GROUP_REQUEST_RESPONSE,CMD_READ_ODO_LOG,(ubyte*)&answer,sizeof(sword)*3);
}
