...
void cn_command_request_path_and_pwm() {
	sword answer[6]; //was 3
	answer[0] = (sword)controller_stat.actual_motion_vector.x;
	answer[1] = (sword)controller_stat.actual_motion_vector.y;
	answer[2] = (sword)(controller_stat.actual_motion_vector.rotation/(ROTATION_PRECISION/64));

  answer[3] = (sword) getMotorPWM(MOTOR1);
  answer[4] = (sword) getMotorPWM(MOTOR2);
  answer[5] = (sword) getMotorPWM(MOTOR3);

  if(getMotorDirection(MOTOR1) == DIRECTION_CW) { answer[3] *= -1; }
  if(getMotorDirection(MOTOR2) == DIRECTION_CW) { answer[4] *= -1; }
  if(getMotorDirection(MOTOR2) == DIRECTION_CW) { answer[5] *= -1; }


	cn_write_response(GROUP_REQUEST_RESPONSE,CMD_PATH_AND_PWM,(ubyte*)&answer,sizeof(sword)*6); //was 3
}
