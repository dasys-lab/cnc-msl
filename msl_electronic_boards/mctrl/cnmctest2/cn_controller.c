...
//	odo_buff_pos = 0;
	...
	controller_params_passive.maxWheelDeccel = 100;
	controller_params_passive.maxWheelAccel  = 100;
...
//controller_stat.motion_goal.rotation = controller_stat.motion_request.rotation;
//END DEBUG

	motionToMotorVector(&controller_stat.motion_goal,temp_rpm);
	temp2 = 1025l; 
	temp3 = 0;
	//Check for RPM max accel / deccel
	//curMaxAccel = (controller_params_active.maxWheelAccel*delta_time)/TIME_TICKS_PER_SECOND;
	//curMaxDeccel = (controller_params_active.maxWheelDeccel*delta_time)/TIME_TICKS_PER_SECOND;
	
	for (i=0; i < 3; i++) {
		temp = temp_rpm[i] - controller_stat.rpm_goal[i]; //the accel vector
		//if (temp == 0) continue;
		if (controller_stat.rpm_goal[i] > 0) {
			if(temp > controller_params_active.maxWheelAccel) { // wheel is accelerating too fast
				temp3 = ((sdword)controller_params_active.maxWheelAccel*1024l+ABS(temp)/2) / ABS(temp); //the fraction of accel to keep (times 1024)
				if (temp2 > temp3) {
					temp2 = temp3;
				}
			} else {
				if (temp < -controller_params_active.maxWheelDeccel) { //wheel is decclerating too fast
					temp3 = ((sdword)controller_params_active.maxWheelDeccel*1024l+ABS(temp)/2) / ABS(temp);
					if (temp2 > temp3) {
						temp2 = temp3;
					}
				}
			...
			if (temp < -controller_params_active.maxWheelAccel) { // wheel is accelerating too fast
					temp3 = ((sdword)controller_params_active.maxWheelAccel*1024l+ABS(temp)/2) / ABS(temp);
					if (temp2 > temp3) {
						temp2 = temp3;
					}
			} 
			else {
				if (temp > controller_params_active.maxWheelDeccel) { //wheel is deccelerating too fast
					temp3 = ((sdword)controller_params_active.maxWheelDeccel*1024l+ABS(temp)/2) / ABS(temp);
					if (temp2 > temp3) {						
						temp2 = temp3;
					}
	...
	if (temp2 >= 1025l) { //all OK
		controller_stat.rpm_goal[0] = temp_rpm[0];
		controller_stat.rpm_goal[1] = temp_rpm[1];
		controller_stat.rpm_goal[2] = temp_rpm[2];
	} else { //scale factor and propagate back to motion_goal
		
		for (i=0; i < 3; i++) {
			temp = (temp_rpm[i] - controller_stat.rpm_goal[i])*temp2;
			controller_stat.rpm_goal[i] += (temp+SIGN2(temp)*512l)/ 1024l;
			
		}
		motorToMotionVector(controller_stat.rpm_goal,&controller_stat.motion_goal);
	}
	
	

//controller_stat.rpm_goal
...
//	printf("RPM %ld %ld %ld\n",controller_stat.actual_rpm[0], controller_stat.actual_rpm[1], controller_stat.actual_rpm[2]);
	...
	motorToMotionVector(controller_stat.actual_rpm,&controller_stat.actual_motion_vector);
...
void motorToMotionVector(sdword* motor,cart_motion_vector* motion) {	
	sdword temp;
	//float temp;
	
	//Calculate Cartesian PathVector:
	...
	temp = motor[2]-motor[1];
	temp *= controller_motor_params.wheel_circ_64;
	//Round:
	//temp += SIGN2(temp)*SIN_120_64*600l;
	temp /= SIN_120_64* 1200l;//sin(120)*8*2*600*8;
	//temp /= 600; //60 seconds, 10 dezi millimeter
	motion->x = temp;	 //64/8 

	temp = -(motor[1]+motor[2]);
	temp += 2*motor[0];
	temp *= controller_motor_params.wheel_circ_64;
	//Round:
	//temp += SIGN2(temp)*57600l;
	temp /= 115200l; //600*3*64
	motion->y = temp; 

	temp = (motor[0]+motor[1]+motor[2]);	
	temp *= controller_motor_params.wheel_circ_64; 
	//ideally it would be:
...
void motionToMotorVector(cart_motion_vector* motion, sdword* motor) {
	...
	//rotation first:
	temp = motion->rotation;
	temp *= controller_motor_params.robot_radius;
	//temp += SIGN2(temp)*controller_motor_params.wheel_circ_4/2; //ROUND
	...
	//temp += SIGN2(temp)*(ROTATION_PRECISION/64); //ROUND
...
//	temp *= SIN_120_8;
//	temp *= 8; //64/8  
	//temp += SIGN2(temp)*controller_motor_params.wheel_circ_64/2l;	//ROUND
	temp /= controller_motor_params.wheel_circ_64;	
	motor[1] -=  temp;
	motor[2] +=	temp;
	...
	//temp += SIGN2(temp)*controller_motor_params.wheel_circ_64/2; //ROUND
	temp /= controller_motor_params.wheel_circ_64;
	motor[0] += temp;
	//temp += SIGN2(temp)*1; //ROUND
	temp /= 2;
	motor[1] -= temp;
	motor[2] -= temp;
}
