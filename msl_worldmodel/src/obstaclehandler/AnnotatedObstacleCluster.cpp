...
namespace msl
{
	...
	bool AnnotatedObstacleCluster::checkAndMerge(AnnotatedObstacleCluster *cluster, double varianceThreshold)
	{
		...
		else
		{
			...
			{
				//TODO hack for scientific testing purposes
				if (std::count(this->supporter->begin(), this->supporter->end(), cluster->supporter->at(i)) == 0)
				{
	...
	{
		stringstream ss;
		ss << "Ident: " << this->ident << "\nCentroid X: " << this->x << " Y: " << this->y << " Radius: "
				<< this->radius << "\nPoints: " << this->numObs;
