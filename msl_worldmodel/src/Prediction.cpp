  ...
  if(ownPos && odo)
  {
    ...
    if (alloAngle > M_PI)
      ...
      if (anglePos)
      {
        ...
        debugAnglePosition << " " << anglePos->at(0) << " " << anglePos->at(1) << " " << anglePos->at(2) << " " << anglePos->at(3) << std::endl << std::flush;
      }
  }
}
...
std::unique_ptr<std::array<double,4>> Prediction::angleAndPosition(int ms)
{
  auto wm = MSLWorldModel::get();
  ...
  std::unique_ptr<std::array<double,4>> returnVal(new std::array<double,4>({angle, x, y, trans}));
