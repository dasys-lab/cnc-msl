...
Ball::Ball(MSLWorldModel *wm, int ringbufferLength) :
		ballPosition(ringbufferLength), ballVelocity(ringbufferLength), ballBuf(
				30), oppBallPossession(ringbufferLength), sharedBallPosition(
				ringbufferLength), ballGuessPosition(ringbufferLength), ballVel3D(
	...
	KICKER_DISTANCE = (*this->sc)["Dribble"]->get<double>("Dribble",
			"KickerDistance", NULL);
	KICKER_ANGLE = M_PI;
	HAVE_BALL_TOLERANCE_DRIBBLE = (*this->sc)["Dribble"]->get<double>("Dribble",
			"HaveBallToleranceDribble", NULL);
	HAVE_BALL_MAX_ANGLE_DELTA = (*this->sc)["Dribble"]->get<double>("Dribble",
			"HaveBallMaxAngleDelta", NULL);
	BALL_DIAMETER = (*this->sc)["Rules"]->get<double>("Rules.BallRadius", NULL)
			* 2;
	...
	MIN_HAVE_BALL_CYCLE = (*this->sc)["Dribble"]->get<int>("Dribble",
			"minHaveBallCycles", NULL);
	AMOUNT_OF_HISTORIZED_CYCLE = (*this->sc)["Dribble"]->get<int>("Dribble",
			"amountOfHistorizedCycles", NULL);
	if (AMOUNT_OF_HISTORIZED_CYCLE < MIN_HAVE_BALL_CYCLE)
	{
		throw;
	}
}
...
shared_ptr<pair<shared_ptr<geometry::CNPoint2D>, double>> Ball::getVisionBallPositionAndCertaincy(
		int index) {
...
shared_ptr<geometry::CNPoint2D> Ball::getEgoBallPosition() {
	...
	if (rawBall == nullptr) {
		...
		if (lsb != nullptr) {
			...
			auto pos = wm->rawSensorData->getOwnPositionVision();
			if (rawBall->first->length() < 2000
					|| (rawBall->first->length() < 4000
							&& rawBall->second > 0.55) || pos == nullptr) {
...
void Ball::updateBallPos(shared_ptr<geometry::CNPoint3D> ballPos,
		shared_ptr<geometry::CNPoint3D> ballVel, double certainty) {
	...
	shared_ptr<InformationElement<geometry::CNPoint2D>> ball = make_shared<
			InformationElement<geometry::CNPoint2D>>(ball2d, time);
	ball->certainty = certainty;
	ballPosition.add(ball);

	shared_ptr<InformationElement<geometry::CNPoint3D>> ball3D = make_shared<
			InformationElement<geometry::CNPoint3D>>(ballPos, time);
	ball->certainty = certainty;
	...
	shared_ptr<InformationElement<geometry::CNPoint3D>> ballVelInfo3D =
			make_shared<InformationElement<geometry::CNPoint3D>>(ballVel, time);
	ball->certainty = certainty;
...
void Ball::updateBallGuess() {
	...
	if (sb != nullptr && sb->first != nullptr && sb->second > 0.0) {
		...
		if (x != nullptr) {
			// 1s
			if (x->getInformation() != nullptr
					&& wm->getTime() < x->timeStamp + 1000000000) {
				guessConf = (double) ((1000000000 + x->timeStamp)
						- wm->getTime()) / 1000000000.0;
				time = x->timeStamp;
	...
	shared_ptr<InformationElement<geometry::CNPoint2D>> bgi = make_shared<
			InformationElement<geometry::CNPoint2D>>(nguess, time);
...
void Ball::updateSharedBall() {
	...
	for (auto &pair : wm->robots->sharedWolrdModelData) {
		...
		shared_ptr<geometry::CNPoint2D> point =
				make_shared<geometry::CNPoint2D>(shwmdata->ball.point.x,
		...
		for (auto &bv : sbvotingList) {
			...
			if (distSqr < thresholdSqr) {
				found = true;
				bv.ballPos->x =
						(bv.ballPos->x * bv.supporters.size() + point->x)
								/ (bv.supporters.size() + 1);
				bv.ballPos->y =
						(bv.ballPos->y * bv.supporters.size() + point->y)
								/ (bv.supporters.size() + 1);
	...
	shared_ptr<InformationElement<geometry::CNPoint2D>> sbi = make_shared<
			InformationElement<geometry::CNPoint2D>>(sb, time);
...
void Ball::updateHaveBall() {
	if (hasBallIteration > 0) {
		...
		haveBallDistanceDynamic = min(haveBallDistanceDynamic + 2,
				HAVE_BALL_TOLERANCE_DRIBBLE);
	} else {
	...
	if (ballPos == nullptr) {
		...
		hasBallIteration = max(
				min(--hasBallIteration, AMOUNT_OF_HISTORIZED_CYCLE), 0);
		this->ballPossessionStatus = (
				this->haveBall() ?
						BallPossessionStatus::HaveBall :
						BallPossessionStatus::NoBallSeen);
	...
	if ((KICKER_DISTANCE + haveBallDistanceDynamic < ballPos->length()
		...
		this->ballPossessionStatus = (
				this->haveBall() ?
						BallPossessionStatus::HaveBall :
						BallPossessionStatus::NotInKickerDistance);
	...
	if (fabs(ballPos->y) > 100) {
		...
		this->ballPossessionStatus = (
				this->haveBall() ?
						BallPossessionStatus::HaveBall :
						BallPossessionStatus::AsideOfKicker);
		// cout << "Ball: Angle Tolerance check failed!" << endl;
		return;
	}
...
void Ball::processSharedWorldModelData(msl_sensor_msgs::SharedWorldInfo &data) {
	if (ballPositionsByRobot.find(data.senderID)
			== ballPositionsByRobot.end()) {
...
shared_ptr<pair<shared_ptr<geometry::CNPoint2D>, double>> Ball::getAlloSharedBallPositionAndCertaincy(
		int index) {
	shared_ptr<pair<shared_ptr<geometry::CNPoint2D>, double>> ret = make_shared<
			pair<shared_ptr<geometry::CNPoint2D>, double>>();
	auto x = sharedBallPosition.getLast(index);
...
bool Ball::haveBallDribble(bool hadBefore) {
	bool ret = (simpleHaveBallDribble(hadBefore)
			&& (hadBefore || this->hasBallIteration > 4));
	this->hadBefore = ret;
	return ret;
}
...
void Ball::processHypothesis() {
	...
	ObservedPoint *opDirected =
			SharedMemoryHelper::getInstance()->readDirectedBallPosition();

	for (int i = 0; i < 10; i++) {
		if (opDirected[i].valid) {
			BallIntegrator::getInstance()->integratePoint(opDirected[i],
					1000.0);
		}
	}
	...
	for (int i = 0; i < ballList->hypothesis.size(); i++) {
		...
		if (ownPosition == nullptr) {
			...
			alloBallPosX += cos(ownPosition->theta) * p.x
					- sin(ownPosition->theta) * p.y;
			alloBallPosY += sin(ownPosition->theta) * p.x
					+ cos(ownPosition->theta) * p.y;

			if (fabs(alloBallPosX)
					< wm->field->getFieldLength() / 2.0 + relFactor
					&& fabs(alloBallPosY)
							< wm->field->getFieldWidth() / 2.0 + relFactor) {
				inField = true;
			}
		}
	...
	ObjectContainer *currBallBuf =
			BallIntegrator::getInstance()->getContainer();
	...
	double dist = sqrt(
			(op.x - mv.point.x) * (op.x - mv.point.x)
					+ (op.y - mv.point.y) * (op.y - mv.point.y));
	mv = ObjectTracker::getInstance()->trackObject(currBallBuf->getPoints(),
			currBallBuf->getSize(), currBallBuf->getStartIndex(),
			currBallBuf->getLastIndex(), 0.3E07);
	bool noValidPoint = false;

	if (fabs(mv.point.x) > 50000.0 && fabs(mv.point.y) > 50000.0) {
		noValidPoint = true;
	}
	ZEstimate ze = BallZTracker::getInstance()->trackObject(
			currBallBuf->getPoints(), currBallBuf->getSize(),
			currBallBuf->getStartIndex(), currBallBuf->getLastIndex());
	geometry::CNPoint3D ballPoint;
	geometry::CNPoint3D ballVelocity;
	MovingObject mv2 = mv;
	mv2.point = allo2Ego(mv.point,
			BallIntegrator::getInstance()->getRefPosition());
	mv2.velocity = allo2Ego(mv.velocity,
			BallIntegrator::getInstance()->getRefPosition());
	...
	if (op.valid && !noValidPoint) {
		...
		auto pos = make_shared<geometry::CNPoint3D>(ballPoint.x, ballPoint.y,
				ballPoint.z);
		auto vel = make_shared<geometry::CNPoint3D>(ballVelocity.x,
				ballVelocity.y, ballVelocity.z);
