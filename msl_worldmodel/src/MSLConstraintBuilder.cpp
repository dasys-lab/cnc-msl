...
shared_ptr<Term> MSLConstraintBuilder::spreadUtil(vector<shared_ptr<TVec>> &points)
{
    ...
    double maxFieldDist = std::sqrt(wm->field->getFieldLength() * wm->field->getFieldLength() + wm->field->getFieldWidth() * wm->field->getFieldWidth());
    ...
    for (int i = 0; i < players.size(); ++i)
    {
        ...
        if (opps != nullptr && opps->size() > 0)
        {
            for (auto opp : *opps)
            {
                ...
                c = c & alica::ConstraintBuilder::ifThen(
                            ((oppDistance->getX() > autodiff::TermBuilder::constant(0)) & (oppDistance->getX() < autodiff::TermBuilder::constant(1))),
                            rel < ((make_shared<Abs>(oppDistance->getY()) -
                                    ((wm->obstacles->getObstacleRadius() + 50.0) / autodiff::TermBuilder::power(p2p->normSquared(), 0.5))) /
                                   oppDistance->getX()));
            }
        }
        ...
        if (mates != nullptr && mates->size() > 0)
        {
            for (auto mate : *mates)
            {
                ...
                c = c & alica::ConstraintBuilder::ifThen(
                            ((mateDistance->getX() > autodiff::TermBuilder::constant(0)) & (mateDistance->getX() < autodiff::TermBuilder::constant(1))),
                            rel < ((make_shared<Abs>(mateDistance->getY()) -
                                    ((wm->obstacles->getObstacleRadius() + 50.0) / autodiff::TermBuilder::power(p2p->normSquared(), 0.5))) /
                                   mateDistance->getX()));
            }
        }
        ...
        if (considerownPos)
        {
            ...
            c = c & alica::ConstraintBuilder::ifThen(
                        ((myPosDistance->getX() > autodiff::TermBuilder::constant(0)) & (myPosDistance->getX() < autodiff::TermBuilder::constant(1))),
                        rel < ((make_shared<Abs>(myPosDistance->getY()) -
                                ((wm->obstacles->getObstacleRadius() + 50.0) / autodiff::TermBuilder::power(p2p->normSquared(), 0.5))) /
                               myPosDistance->getX()));
        }
    }
...
shared_ptr<Term> MSLConstraintBuilder::outsideRectangle(shared_ptr<TVec> lowerRightCorner, shared_ptr<TVec> upperLeftCorner, vector<shared_ptr<TVec>> &points)
{
...
shared_ptr<Term> MSLConstraintBuilder::insideRectangle(shared_ptr<TVec> lowerRightCorner, shared_ptr<TVec> upperLeftCorner, vector<shared_ptr<TVec>> &points)
{
    shared_ptr<Term> c = TermBuilder::boundedRectangle(points[0], lowerRightCorner, upperLeftCorner, Term::getConstraintSteepness());
...
shared_ptr<Term> MSLConstraintBuilder::outsideTriangle(shared_ptr<TVec> a, shared_ptr<TVec> b, shared_ptr<TVec> c, double tolerance,
                                                       vector<shared_ptr<TVec>> &points)
{
    ...
    for (int i = 0; i < points.size(); i++)
    {
        ...
        pConsts = ((a2p->getX() * a2b->getY() - a2p->getY() * a2b->getX()) / autodiff::TermBuilder::power(a2p->normSquared(), 0.5)) >
                  autodiff::TermBuilder::constant(tolerance);
        pConsts = pConsts |
                  ((b2p->getX() * b2c->getY() - b2p->getY() * b2c->getX()) / autodiff::TermBuilder::power(b2p->normSquared(), 0.5)) >
                      autodiff::TermBuilder::constant(tolerance);
        pConsts = pConsts |
                  ((c2p->getX() * c2a->getY() - c2p->getY() * c2a->getX()) / autodiff::TermBuilder::power(c2p->normSquared(), 0.5)) >
                      autodiff::TermBuilder::constant(tolerance);
...
shared_ptr<Term> MSLConstraintBuilder::insideTriangle(shared_ptr<TVec> a, shared_ptr<TVec> b, shared_ptr<TVec> c, double tolerance,
                                                      vector<shared_ptr<TVec>> &points)
{
    shared_ptr<TVec> a2b = b - a;
    shared_ptr<TVec> b2c = c - b;
...
shared_ptr<Term> MSLConstraintBuilder::ownPenaltyRules(shared_ptr<TVec> ballT, int executerIdx, vector<shared_ptr<TVec>> &fieldPlayers)
{
    ...
    std::copy_if(std::begin(fieldPlayers), std::end(fieldPlayers), std::back_inserter(passivPlayers),
                 [&](shared_ptr<TVec> v) { return v != fieldPlayers[executerIdx]; });
    ...
    {
        appliedRules =
            appliedRules & alica::ConstraintBuilder::ifThenElse(ballT->getX() > (autodiff::TermBuilder::constant(1000.0)),
                                                                insideArea(Areas::OwnHalf, passivPlayers), insideArea(Areas::OppHalf, passivPlayers));
    }
    else
    {
...
shared_ptr<Term> MSLConstraintBuilder::ownPenaltyAreaDistanceExceptionRule(shared_ptr<TVec> ballT, vector<shared_ptr<TVec>> &fieldPlayers)
{
    ...
    for (int i = 0; i < fieldPlayers.size(); i++)
    {
        ...
        if (ballT != nullptr)
        {
            minDistConstrains = minDistConstrains & alica::ConstraintBuilder::ifThen(outsideArea(Areas::OwnPenaltyArea, fieldPlayers.at(i)),
