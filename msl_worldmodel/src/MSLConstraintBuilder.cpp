...
namespace msl
{
     ...
     */
    shared_ptr<Term> MSLConstraintBuilder::lineUpUtil(shared_ptr<geometry::CNPointAllo> norm, double d,
    ...
    shared_ptr<Term> MSLConstraintBuilder::outsideCorridor(shared_ptr<geometry::CNPointAllo> a,
                                                           shared_ptr<geometry::CNPointAllo> b, double width,
                                                           vector<shared_ptr<TVec>> &points)
    {
        shared_ptr<Term> c = autodiff::LTConstraint::TRUE;
    ...
    shared_ptr<Term> MSLConstraintBuilder::insideCorridor(geometry::CNPointAllo a, geometry::CNPointAllo b,
    ...
    shared_ptr<Term> MSLConstraintBuilder::outsideConvex(vector<shared_ptr<TVec>> &shell, double tolerance,
                                                         vector<shared_ptr<TVec>> &points)
    {
        ...
        for (int i = 0; i < points.size(); i++)
        {
            ...
            for (int j = 0; j < shellVec.size(); j++)
            {
                ...
                outsideConstraints = outsideConstraints
                        | alica::ConstraintBuilder::rightOf(shellVec.at(j), points.at(i));
            }
    ...
    shared_ptr<Term> MSLConstraintBuilder::insideConvex(vector<shared_ptr<TVec>> &shell, double tolerance,
                                                        vector<shared_ptr<TVec>> &points)
    {
        vector<shared_ptr<TVec>> shellVec;
    ...
    {
        geometry::CNPointAllo lowerRightCornerP;
        geometry::CNPointAllo upperLeftCornerP;
        resolveArea(area, lowerRightCornerP, upperLeftCornerP);
        shared_ptr<TVec> lowerRightCorner = make_shared<TVec>(
                initializer_list<double> {lowerRightCornerP.x - tolerance, lowerRightCornerP.y - tolerance});
        shared_ptr<TVec> upperLeftCorner = make_shared<TVec>(
                initializer_list<double> {upperLeftCornerP.x + tolerance, upperLeftCornerP.y + tolerance});
...
//	shared_ptr<geometry::CNPoint2D> MSLConstraintBuilder::oppLeftSurCornerP = wm->field.posULSurrounding();
    ...
    void MSLConstraintBuilder::resolveArea(Areas area, geometry::CNPointAllo& lowerRightCorner,
