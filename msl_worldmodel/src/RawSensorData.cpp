...
shared_ptr<geometry::CNPoint2D> RawSensorData::getEgoGoalMid(int index)
{
	auto point = this->egoGoalMid.getLast(index);
    if (point == nullptr || wm->getTime() - point->timeStamp > maxInformationAge)
    {
        return nullptr;
    }
    return point->getInformation();
}
...
void RawSensorData::processGoalMid(geometry_msgs::Point point)
{
    shared_ptr<geometry::CNPoint2D> goalMid = make_shared<geometry::CNPoint2D>(point.x, point.y);
    shared_ptr<InformationElement<geometry::CNPoint2D>> goalPoint = make_shared<InformationElement<geometry::CNPoint2D>>(goalMid, wm->getTime());
    this->egoGoalMid.add(goalPoint);
}
