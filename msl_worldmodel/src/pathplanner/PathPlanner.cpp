...
PathPlanner::PathPlanner(MSLWorldModel *wm, int count)
{
        ...
        this->voronoiDiagrams.reserve(10);
    {
        ...
        voi->setVoronoi(make_shared<VoronoiDiagram>((DelaunayTriangulation) this->artificialObjectNet->getVoronoi()->dual()));
    ...
    this->corridorWidthDivisorBall = (*this->sc)["PathPlanner"]->get<double>("PathPlanner", "corridorWidthDivisorBall", NULL);
    ...
    this->additionalBallCorridorWidth = (*this->sc)["PathPlanner"]->get<double>("PathPlanner", "additionalBallCorridorWidth", NULL);
...
shared_ptr<vector<shared_ptr<geometry::CNPoint2D>>> PathPlanner::plan(shared_ptr<VoronoiNet> voronoi, shared_ptr<geometry::CNPoint2D> startPos,
...
shared_ptr<vector<shared_ptr<geometry::CNPoint2D>>> PathPlanner::aStarSearch(shared_ptr<VoronoiNet> voronoi, shared_ptr<geometry::CNPoint2D> startPos,
...
bool PathPlanner::isAdmissableEdge(VoronoiDiagram::Halfedge_around_vertex_circulator incidentHalfEdge, shared_ptr<geometry::CNPoint2D> startPos,
...
void PathPlanner::expandNode(shared_ptr<SearchNode> currentNode, shared_ptr<vector<shared_ptr<SearchNode>>> open,
                             shared_ptr<vector<shared_ptr<SearchNode>>> closed, shared_ptr<geometry::CNPoint2D> startPos, shared_ptr<geometry::CNPoint2D> goal,
                             shared_ptr<IPathEvaluator> pathEvaluator, shared_ptr<VoronoiNet> voronoi)
{
...
bool PathPlanner::checkGoalVerticesReached(shared_ptr<vector<shared_ptr<Vertex>>> closestVerticesToGoal, shared_ptr<SearchNode> currentNode)
{
...
bool PathPlanner::closeOppToBallCheck(shared_ptr<VoronoiNet> voronoi, shared_ptr<geometry::CNPoint2D> currentPos, shared_ptr<geometry::CNPoint2D> goal,
...
bool PathPlanner::corridorCheck(shared_ptr<geometry::CNPoint2D> currentPos, shared_ptr<geometry::CNPoint2D> goal, shared_ptr<geometry::CNPoint2D> obstaclePoint,
                                double obstacleRadius)
{
    ...
    shared_ptr<geometry::CNPoint2D> p1 = make_shared<geometry::CNPoint2D>(currentPos->x + (this->robotRadius + this->additionalCorridorWidth) * dy,
                                                                          currentPos->y - (this->robotRadius + this->additionalCorridorWidth) * dx);
    shared_ptr<geometry::CNPoint2D> p2 = make_shared<geometry::CNPoint2D>(currentPos->x - (this->robotRadius + this->additionalCorridorWidth) * dy,
                                                                          currentPos->y + (this->robotRadius + this->additionalCorridorWidth) * dx);
    shared_ptr<geometry::CNPoint2D> p3 = make_shared<geometry::CNPoint2D>(
        goal->x + std::max(this->robotRadius + this->additionalCorridorWidth, length / this->corridorWidthDivisor + this->additionalCorridorWidth) * dy,
        goal->y - std::max(this->robotRadius + this->additionalCorridorWidth, length / this->corridorWidthDivisor + this->additionalCorridorWidth) * dx);
    ...
    return obstaclePoint != nullptr &&
           (geometry::distancePointToLine(p2, p4, obstaclePoint) < this->robotRadius && geometry::distancePointToLine(p4, p3, obstaclePoint) < 0.0 &&
            geometry::distancePointToLine(p3, p1, obstaclePoint) < this->robotRadius && geometry::distancePointToLine(p1, p2, obstaclePoint) < 0.0);
    // return result
    // return obstaclePoint != nullptr && geometry::isInsidePolygon(points, obstaclePoint);
}

bool PathPlanner::corridorCheckBall(shared_ptr<geometry::CNPoint2D> currentPos, shared_ptr<geometry::CNPoint2D> goal,
                                    shared_ptr<geometry::CNPoint2D> obstaclePoint, double obstacleRadius)
{
    ...
    dy /= dist;
    // calculate corridor corner points
    shared_ptr<geometry::CNPoint2D> p1 = make_shared<geometry::CNPoint2D>(currentPos->x + (this->robotRadius) * dy, currentPos->y - (this->robotRadius) * dx);
    shared_ptr<geometry::CNPoint2D> p2 = make_shared<geometry::CNPoint2D>(currentPos->x - (this->robotRadius) * dy, currentPos->y + (this->robotRadius) * dx);
    shared_ptr<geometry::CNPoint2D> p3 =
        make_shared<geometry::CNPoint2D>(goal->x +
                                         ...
                                         goal->y -
                                             std::max(wm->ball->getBallDiameter() / 2 + this->additionalBallCorridorWidth,
                                                      length / this->corridorWidthDivisorBall + this->additionalBallCorridorWidth) *
                                                 dx);
    shared_ptr<geometry::CNPoint2D> p4 =
        make_shared<geometry::CNPoint2D>(goal->x -
                                             std::max(wm->ball->getBallDiameter() / 2 + this->additionalBallCorridorWidth,
                                                      length / this->corridorWidthDivisorBall + this->additionalBallCorridorWidth) *
                                                 dy,
...
shared_ptr<vector<shared_ptr<geometry::CNPoint2D>>> PathPlanner::getArtificialFieldSurroundingObs()
{
    ...
    int lengthInterval = (int)(baseSize + ((int)(wm->field->getFieldLength() + 2000) % baseSize) / (int)((int)(wm->field->getFieldLength() + 2000) / baseSize));
    int widthInterval = (int)(baseSize + ((int)(wm->field->getFieldWidth() + 2000) % baseSize) / (int)((int)(wm->field->getFieldWidth() + 2000) / baseSize));
