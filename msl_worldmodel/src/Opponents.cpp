...
namespace msl
{
    ...
    Opponents::Opponents(MSLWorldModel *wm, int ringBufferLength) :
            opponentsAlloClustered(ringBufferLength), opponentsEgoClustered(ringBufferLength), opponentsAlloClusteredNoTeamMates(
                    ringBufferLength)
    {
        this->wm = wm;
    ...
    shared_ptr<vector<shared_ptr<geometry::CNRobot> > > Opponents::getOpponentsAlloClusteredNoTeam(int index)
    {
        auto x = opponentsAlloClusteredNoTeamMates.getLast(index);
        if (x == nullptr || wm->getTime() - x->timeStamp > maxInformationAge)
        {
            return nullptr;
        }
    ...
    void Opponents::processOpponentsAlloClusteredNoTeam(
            shared_ptr<vector<shared_ptr<geometry::CNRobot> > > opponentsAlloClusteredNoTeam)
    {
        shared_ptr<InformationElement<vector<shared_ptr<geometry::CNRobot>>> > o=
    make_shared<InformationElement<vector<shared_ptr<geometry::CNRobot>>>>(opponentsAlloClusteredNoTeam, wm->getTime());
    o->certainty = 1;

    this->opponentsAlloClusteredNoTeamMates.add(o);
