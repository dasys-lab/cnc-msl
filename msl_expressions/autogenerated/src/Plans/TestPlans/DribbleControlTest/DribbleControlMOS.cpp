...
void DribbleControlMOS::run(void *msg)
{
    ...
    if (plannedMotion != nullptr)
    {
        ...
        if (robotVel < 100.0 &&
            ((plannedMotion->motion.angle < M_PI / 4.0 && plannedMotion->motion.angle > -M_PI / 4.0) || plannedMotion->motion.angle > M_PI * 7.0 / 4.0) &&
...
void DribbleControlMOS::initialiseParameters()
{
    ...
    this->forwConst = sqrt((sin(M_PI / 2.0 - 0.82)) * sin(M_PI / 2.0 - 0.82) + (sin(0.75) * cos(M_PI / 2.0 - 0.82)) * (sin(0.75) * cos(M_PI / 2.0 - 0.82))) /
                      cos(0.349); // ballVel -> ArmVel for forward
...
void DribbleControlMOS::getBallPath(double translation, double angle, double rotation, double &velX, double &velY)
{
    ...
    // special case where y velocity and rotation result in motion around the liing ball (sign(velY)!=sign(rot))
    // TODO
    if (geometry::sgn(velYTemp) != geometry::sgn(rotation) && fabs(velYTemp) > 100 && fabs(rotation) > 0.3)
    {
        velY = velYTemp;
    }
    else
    ...
    if (velXTemp < 0.0)
    {
        ...
        if (this->jumpPlanned || this->transTolerance <= fabs(translation - this->translationOld) || this->rotTolerance <= fabs(rotation - this->rotationOld))
        // || angleTolerance <= fabs(angle - angleOld))
        {
            // powerFactor decays over the iterations
            cout << "DribbleControlMOS::getBallPath: Jump detected" << fabs(translation - this->translationOld) << " " << fabs(rotation - rotationOld) << " "
                 << fabs(angle - this->angleOld) << endl;
