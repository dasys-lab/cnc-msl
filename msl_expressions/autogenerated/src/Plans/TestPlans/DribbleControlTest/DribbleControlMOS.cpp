...
void DribbleControlMOS::initialiseParameters() {
	...
	testBehaviour = (*sc)["DribbleAlround"]->get<int>(
			"DribbleAlround.testBehaviour", NULL);
	...
	velToInput = (*sc)["DribbleAlround"]->get<double>(
			"DribbleAlround.velToInput", NULL);
	staticUpperBound = (*sc)["DribbleAlround"]->get<double>(
			"DribbleAlround.staticUpperBound", NULL);
	staticMiddleBound = (*sc)["DribbleAlround"]->get<double>(
			"DribbleAlround.staticMiddleBound", NULL);
	staticLowerBound = (*sc)["DribbleAlround"]->get<double>(
			"DribbleAlround.staticLowerBound", NULL);
	staticNegVelX = (*sc)["DribbleAlround"]->get<double>(
			"DribbleAlround.staticNegVelX", NULL);
	rBallRobot = (*sc)["DribbleAlround"]->get<double>(
			"DribbleAlround.rBallRobot", NULL);
	...
	epsilonY = (*sc)["DribbleAlround"]->get<double>("DribbleAlround.epsilonY",
			NULL);
	velYFactor = (*sc)["DribbleAlround"]->get<double>(
			"DribbleAlround.velYFactor", NULL);
	powerFactor = (*sc)["DribbleAlround"]->get<double>(
				"DribbleAlround.powerFactor", NULL);
	transTolerance = (*sc)["DribbleAlround"]->get<double>(
			"DribbleAlround.transTolerance", NULL);
	rotTolerance = (*sc)["DribbleAlround"]->get<double>(
			"DribbleAlround.rotTolerance", NULL);
	angleTolerance = (*sc)["DribbleAlround"]->get<double>(
			"DribbleAlround.angleTolerance", NULL);
	...
	forwConst = sqrt(
			(sin(M_PI / 2 - 0.82)) * sin(M_PI / 2 - 0.82)
					+ (sin(0.75) * cos(M_PI / 2 - 0.82))
							* (sin(0.75) * cos(M_PI / 2 - 0.82))) / cos(0.349); //ballVel -> ArmVel for forward
...
void DribbleControlMOS::getBallPath(double translation, double angle,
	...
	cout << "velY = " << velXTemp << "+ 3/4 * " << rBallRobot << " * "
			<< rotation;
...
double DribbleControlMOS::getLeftArmVelocity(double ballVelocity,
		double ballAngle) {
	double sec0 = -M_PI;
...
//linear interpolation of the constants in the 8 sectors
	...
	if (ballAngle <= sec1) {
		...
		cout << "sec2" << endl;
		angleConst = sidewConst
				+ (ballAngle - sec2) * (diagConst - sidewConst) / (sec3 - sec2);
	} else if (ballAngle <= sec4) {
		cout << "sec3" << endl;
		angleConst = diagConst
				+ (ballAngle - sec3) * (forwConst - diagConst) / (sec4 - sec3);
	} else if (ballAngle <= sec5) {
		cout << "sec4" << endl;
		angleConst = forwConst
				+ (ballAngle - sec4) * (-forwConst) / (sec5 - sec4);
		...
		cout << "sec6" << endl;
		angleConst = -sidewConst
				+ (ballAngle - sec6) * (sidewConst - diagConst) / (sec7 - sec6);
	} else if (ballAngle >= sec7) {
		// rotate left
		cout << "sec7" << endl;
		angleConst = -diagConst
				+ (ballAngle - sec7) * (-forwConst + diagConst) / (sec8 - sec7);
	}
...
double DribbleControlMOS::getRightArmVelocity(double ballVelocity,
		double ballAngle) {
	double sec0 = -M_PI;
	double sec1 = -M_PI / 2 - phi;
	double sec2 = -M_PI / 2;
...
//linear interpolation of the constants in the 8 sectors
	...
	if (ballAngle <= sec1) {
		// rotate right
		angleConst = -forwConst
				+ (ballAngle - sec0) * (-diagConst + forwConst) / (sec1 - sec0);
	} else if (ballAngle <= sec2) {
		angleConst = -diagConst
				+ (ballAngle - sec1) * (-sidewConst + diagConst)
						/ (sec2 - sec1);
	} else if (ballAngle <= sec3) {
		angleConst = -sidewConst
				+ (ballAngle - sec2) * (sidewConst) / (sec3 - sec2);
	} else if (ballAngle <= sec4) {
		angleConst = (ballAngle - sec3) * forwConst / (sec4 - sec3);
	} else if (ballAngle <= sec5) {
		angleConst = forwConst
				+ (ballAngle - sec4) * (diagConst - forwConst) / (sec5 - sec4);
	} else if (ballAngle <= sec6) {
		angleConst = diagConst
				+ (ballAngle - sec5) * (sidewConst - diagConst) / (sec6 - sec5);
	} else if (ballAngle <= sec7) {
		angleConst = sidewConst
				+ (ballAngle - sec6) * (-sidewConst) / (sec7 - sec6);
	} else if (ballAngle >= sec7) {
		// rotate left
