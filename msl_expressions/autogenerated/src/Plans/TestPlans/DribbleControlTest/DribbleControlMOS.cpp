...
void DribbleControlMOS::run(void *msg)
{
    ...
    {
    	//prevents unnecessary output while using the simulator
...
void DribbleControlMOS::initialiseParameters()
{
    ...
    forwConst = sqrt((sin(M_PI / 2 - 0.82)) * sin(M_PI / 2 - 0.82) + (sin(0.75) * cos(M_PI / 2 - 0.82)) * (sin(0.75) * cos(M_PI / 2 - 0.82))) /
...
void DribbleControlMOS::sendWheelSpeed(msl_actuator_msgs::BallHandleCmd &msgback)
{
    ...
    if (this->wheelSpeedLeftOld < -2000 && this->wheelSpeedRightOld < -2000 && msgback.leftMotor > this->wheelSpeedLeftOld &&
...
void DribbleControlMOS::getBallPath(double translation, double angle, double rotation, double &velX, double &velY)
{
    ...
    if (velXTemp < 0)
    {
        ...
        if (transTolerance <= fabs(translation - translationOld) && rotTolerance <= fabs(rotation - rotationOld) && angleTolerance <= fabs(angle - angleOld))
        {
            ...
            decayFactor = decayFactor <= 1 ? 1 : decayFactor - 0.05;
        }
        else
        {
            decayFactor = 1;
    ...
    else if (velXTemp <= staticMiddleBound && velXTemp >= staticLowerBound && velYTemp <= staticMiddleBound && velYTemp >= staticLowerBound &&
