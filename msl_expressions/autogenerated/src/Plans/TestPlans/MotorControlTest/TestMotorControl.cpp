...
namespace alica
{
    ...
    void TestMotorControl::run(void* msg)
    {
    	...
    	if (straight)
    	{
    		motorMsg.motion.angle = atan2(relGoalY,relGoalX) - wm->rawSensorData->getOwnPositionMotion()->theta + start->theta;
    	...
    	else
    	{
    		...
    		motorMsg.motion.rotation = testSpeed *2 * sin(relGoalRot/2) /  sqrt(relGoalX*relGoalX+relGoalY*relGoalY);
    	}
    ...
    void TestMotorControl::initialiseParameters()
    {
    	...
    	abortTime = (*sc)["MotorControlTest"]->get<double>("MotorControlTest.abortTime", NULL) * sqrt(relGoalX*relGoalX+relGoalY*relGoalY) / testSpeed ;
