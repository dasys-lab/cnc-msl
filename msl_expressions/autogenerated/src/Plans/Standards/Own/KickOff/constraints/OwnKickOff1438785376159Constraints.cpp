...
void Constraint1469455112199::getConstraint(shared_ptr<ProblemDescriptor> c, shared_ptr<RunningPlan> rp)
{
    ...
    if (r1->size() == 0 || r1 == nullptr)
    {
        ...
        // this->logger->log("GenericExecute_OwnKickOff", "No Reciver Assigned!?!?!?", LogLevels::error);
    ...
    else
    {
        for (int id : *r1)
        {
            ...
            {
                futurePasser = make_shared<autodiff::TVec>(initializer_list<double>{futurePasserPos->x, futurePasserPos->y});
            }
            else
            {
                cout << "GenericExecute: No Receiver Position in WM" << endl;
                // this->logger->log("GenericExecute_OwnKickOff", "No Reciver Position in WM", LogLevels::error);
    ...
    if (oppsVecs.size() > 0)
    {
        for (int i = 0; i < oppsVecs.size(); i++)
        {
            ...
            constraint = constraint & ConstraintBuilder::distanceSqr(target, oppsVecs[i]) > autodiff::TermBuilder::constant(1500 * 1500); // get away from opps

            minOppDist = make_shared<autodiff::Min>(minOppDist, ConstraintBuilder::distance(target, oppsVecs.at(i)));
        }
    }
    else
    {
    ...
    if (executor.size() > 0)
    {
        ...
        if (mateVec.size() > 0)
        {
            ...
            {
                constraint =
                    constraint & ConstraintBuilder::distanceSqr(target, mateVec[i]) > autodiff::TermBuilder::constant(2000 * 2000); // get away from mates
            }
        }
    }
    ...
    util = util + 100000 - (make_shared<autodiff::Abs>(targetDist - ConstraintBuilder::distance(target, futurePasser))) + minOppDist;
