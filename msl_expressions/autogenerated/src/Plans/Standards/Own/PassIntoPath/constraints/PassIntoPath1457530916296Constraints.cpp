...
void Constraint1457531039142::getConstraint(shared_ptr<ProblemDescriptor> c, shared_ptr<RunningPlan> rp)
{
    ...
    if (opps != nullptr)
    {
        for (auto opp : *opps)
        {
            ...
            if (opp->x < (-wm->field->getFieldLength() / 2 + wm->field->getPenaltyAreaLength()) ||
                opp->x > (wm->field->getFieldLength() / 2 - wm->field->getPenaltyAreaLength()) || opp->y > wm->field->getFieldWidth() / 2 - 1000 ||
            ...
            else if (ballPose != nullptr && ballPose->distanceTo(opp) > 1100 && ballPose->distanceTo(opp) < wm->field->getFieldLength() * 0.35)
            {
                blockPositions.push_back(make_shared<TVec>(initializer_list<double>{blockingPos->x, blockingPos->y}));
            }
            else
            {
                lossGoodBlockOpponents.push_back(make_shared<TVec>(initializer_list<double>{blockingPos->x, blockingPos->y}));
            }
        }
    }
    ...
    if (blocker.size() > 0)
    {
        ...
        for (int k = 0; k < blocker.size(); k++)
        {
            ...
            {
                blockUtil = make_shared<autodiff::Max>(blockUtil, 10 * (1 - ((ConstraintBuilder::distanceSqr(blocker.at(k), blockPositions.at(j))) /
    ...
    /*
     ...
     c.Constraint &= ConstraintBuilder.IfThen(passVec.X<0, Math.Tan(Math.PI/4) > new AD.Abs(Tan - ((ball.X + field.PosOwnGoalMid.X)/(ball.Y +
     field.PosOwnGoalMid.Y))));
