...
namespace alicaAutogenerated
{
     ...
     * - EP:1457530953060 : StandardReceiver (1238601842183)
     * - EP:1457531047960 : Blocker (1432209050494)
     *
     * States:
     * - Align (1457530916297)
     * - Align (1457531111752)
     ...
     */
    void Constraint1457531039142::getConstraint(shared_ptr<ConstraintDescriptor> c, shared_ptr<RunningPlan> rp)
    {
		...
		c->getStaticRanges()->at(0).at(0) = -msl::MSLFootballField::FieldLength / 2;
		c->getStaticRanges()->at(0).at(1) = msl::MSLFootballField::FieldLength / 2;
		c->getStaticRanges()->at(1).at(0) = -msl::MSLFootballField::FieldWidth / 2;
		c->getStaticRanges()->at(1).at(1) = msl::MSLFootballField::FieldWidth / 2;
        ...
        {
            c->getDomainRanges()->at(0).at(i).at(0).at(0) = -msl::MSLFootballField::FieldLength / 2 + 1000;
            c->getDomainRanges()->at(0).at(i).at(0).at(1) = msl::MSLFootballField::FieldLength / 2 - 1000;
            c->getDomainRanges()->at(0).at(i).at(1).at(0) = -msl::MSLFootballField::FieldWidth / 2 + 1000;
            c->getDomainRanges()->at(0).at(i).at(1).at(1) = msl::MSLFootballField::FieldWidth / 2 - 1000;
        ...
        if (opps != nullptr)
        {
            for (auto opp : *opps)
            {
            	...
            	if(opp->x < ( -msl::MSLFootballField::FieldLength / 2 + msl::MSLFootballField::PenaltyAreaLength ) ||
						opp->x > (msl::MSLFootballField::FieldLength / 2-msl::MSLFootballField::PenaltyAreaLength) ||
						opp->y > msl::MSLFootballField::FieldWidth/2-1000 ||
						opp->y < -msl::MSLFootballField::FieldWidth/2+1000) continue;
                ...
                } else if(ballPose != nullptr && ballPose->distanceTo(opp) > 1100 && ballPose->distanceTo(opp) < msl::MSLFootballField::FieldLength*0.35) {
                	blockPositions.push_back(
                            make_shared < TVec > (initializer_list<double> {blockingPos->x, blockingPos->y}));
                } else {
                	lossGoodBlockOpponents.push_back(
                            make_shared < TVec > (initializer_list<double> {blockingPos->x, blockingPos->y}));
                }
            }
        }
