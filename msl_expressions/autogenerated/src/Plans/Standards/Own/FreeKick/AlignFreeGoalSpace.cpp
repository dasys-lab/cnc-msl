...
void AlignFreeGoalSpace::initialiseParameters()
{
    ...
    alloLeftAimPoint = make_shared<geometry::CNPoint2D>(wm->field->getFieldLength() / 2 + ballDiameter, goalLineLength / 2 - ballDiameter);
    alloRightAimPoint = make_shared<geometry::CNPoint2D>(wm->field->getFieldLength() / 2 + ballDiameter, -goalLineLength / 2 + ballDiameter);

    // Create points for rectangle check
    frontLeft = make_shared<geometry::CNPoint2D>((wm->field->getFieldLength() - (4 * robotRadius)) / 2, goalLineLength / 2);
    frontRight = make_shared<geometry::CNPoint2D>((wm->field->getFieldLength() - (4 * robotRadius)) / 2, -goalLineLength / 2);
    /*PROTECTED REGION END*/
}
/*PROTECTED REGION ID(methods1467039782450) ENABLED START*/ // Add additional methods here
msl_actuator_msgs::MotionControl AlignFreeGoalSpace::alignToPointWithBall(shared_ptr<geometry::CNPoint2D> egoAlignPoint,
