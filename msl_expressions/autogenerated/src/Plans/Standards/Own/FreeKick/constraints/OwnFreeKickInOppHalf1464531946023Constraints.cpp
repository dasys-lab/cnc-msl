...
namespace alicaAutogenerated
{
     ...
     * forall agents in StandInsideOppPenalty let v = [x, y] 

     * forall agents in StandCloseToOppPenalty let v = [x, y] 

     */
    void Constraint1465474610518::getConstraint(shared_ptr<ConstraintDescriptor> c, shared_ptr<RunningPlan> rp)
    {
        /*PROTECTED REGION ID(cc1465474610518) ENABLED START*/
        ...
        for (int i = 0; i < domainVariables->at(0)->size(); i++)
        {
            ...
            c->getDomainRanges()->at(0).at(i).at(0).at(1) = wm->field->getFieldLength() / 2
                    - wm->field->getGoalAreaLength();
            if (ballPose->y > 0)
            {
                c->getDomainRanges()->at(0).at(i).at(1).at(0) = 500;
            ...
            else
            {
                ...
                c->getDomainRanges()->at(0).at(i).at(1).at(1) = -500;
//			c->getDomainRanges()->at(0).at(i).at(1).at(0) = wm->field->getPenaltyAreaWidth() / 2;
//			c->getDomainRanges()->at(0).at(i).at(1).at(1) = -wm->field->getPenaltyAreaWidth() / 2;
            }
        ...
        for (int i = 0; i < domainVariables->at(1)->size(); i++)
        {
            ...
            if (ballPose->y > 0)
            {
                ...
                c->getDomainRanges()->at(1).at(i).at(1).at(1) = 0;
            }
            else
            {
                c->getDomainRanges()->at(1).at(i).at(1).at(0) = 0;
        ...
        if (opps != nullptr)
        {
            for (auto opp : *opps)
            {
                ...
                //add blocking position but do not stand in front of goalie
                shared_ptr < geometry::CNPoint2D > blockingPos;
                if (ballPose != nullptr && !wm->field->isInsideOppGoalArea(opp, 100))
                {
                    blockingPos = opp + (ballPose - opp)->normalize() * 700;
                }
                else if (!wm->field->isInsideOppGoalArea(opp, 100))
                {
