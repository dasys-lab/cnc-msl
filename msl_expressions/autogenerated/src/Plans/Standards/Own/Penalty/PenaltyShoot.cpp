...
void PenaltyShoot::run(void *msg)
{
    ...
    shared_ptr<geometry::CNPoint2D> alloLeftAimPoint =
        make_shared<geometry::CNPoint2D>((wm->field->getFieldLength() / 2) + ballDiameter, (goalWidth / 2) - (aimOffset * ballDiameter));
    shared_ptr<geometry::CNPoint2D> alloRightAimPoint =
        make_shared<geometry::CNPoint2D>((wm->field->getFieldLength() / 2) + ballDiameter, -(goalWidth / 2) + (aimOffset * ballDiameter));

    // Create points for rectangle check
    shared_ptr<geometry::CNPoint2D> frontLeft = make_shared<geometry::CNPoint2D>((wm->field->getFieldLength() - (4 * robotRadius)) / 2, goalWidth / 2);
    shared_ptr<geometry::CNPoint2D> frontRight = make_shared<geometry::CNPoint2D>((wm->field->getFieldLength() - (4 * robotRadius)) / 2, -goalWidth / 2);
...
msl_actuator_msgs::MotionControl PenaltyShoot::alignToPointWithBall(shared_ptr<geometry::CNPoint2D> egoAlignPoint, shared_ptr<geometry::CNPoint2D> egoBallPos,
