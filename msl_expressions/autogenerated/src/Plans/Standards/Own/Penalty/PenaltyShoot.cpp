...
void PenaltyShoot::run(void *msg)
{
    ...
    auto alloLeftAimPoint = geometry::CNPointAllo((wm->field->getFieldLength() / 2) + ballDiameter,
                                                  (goalWidth / 2) - (aimOffset * ballDiameter));
    auto alloRightAimPoint = geometry::CNPointAllo((wm->field->getFieldLength() / 2) + ballDiameter,
                                                   -(goalWidth / 2) + (aimOffset * ballDiameter));

    // Create points for rectangle check
    auto frontLeft = geometry::CNPointAllo((wm->field->getFieldLength() - (4 * robotRadius)) / 2, goalWidth / 2);
    auto frontRight = geometry::CNPointAllo((wm->field->getFieldLength() - (4 * robotRadius)) / 2, -goalWidth / 2);
...
msl_actuator_msgs::MotionControl PenaltyShoot::alignToPointWithBall(geometry::CNPointEgo egoAlignPoint,
