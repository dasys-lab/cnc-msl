...
namespace alica
{
    ...
    void ReceiveInOppHalf::run(void* msg)
    {
        ...
        for (auto opp : *opps)
        {
            double distToLine = geometry::distancePointToLineSegment(opp->x, opp->y, 2000, this->yCoordOfReceiver,
        ...
        this->alloTarget = make_shared < geometry::CNPoint2D
                > (this->wm->field->getFieldLength() / 4, this->yCoordOfReceiver);
