	...
	void Constraint1462361418213::getConstraint(shared_ptr<ConstraintDescriptor> c, shared_ptr<RunningPlan> rp)
	{
		...
		if (blocker.size() > 0)
		{
			...
			if (opps != nullptr && opps->size() > 0)
			{
				...
				shared_ptr < TVec > blockedOpp = make_shared < TVec > (initializer_list<double> {nearestOpp->x, nearestOpp->y});
		...
		if (receiver.size() > 0)
		{
//			double distToLine = geometry::distancePointToLineSegment(opp->x, opp->y, field->x + 2, 5, field->x +6);
//			msl::MSLWorldModel::get()->field
		...
		//cout << "_------------------_" << endl;
//		cout << constraint->aggregateConstants()->toString() << endl;
//		cout << "_------------------_" << endl;
