...
namespace alicaAutogenerated
{
    ...
    void Constraint1462361418213::getConstraint(shared_ptr<ProblemDescriptor> c, shared_ptr<RunningPlan> rp)
    {
        ...
        for (int quantorCount = 0; quantorCount < domainVariables->size(); quantorCount++)
        {
            for (int i = 0; i < domainVariables->at(quantorCount)->size(); i++)
            {
                ...
                auto vec = make_shared<TVec>(initializer_list<shared_ptr<Term>> {
                        dynamic_pointer_cast<autodiff::Variable>(domainVariables->at(quantorCount)->at(i)->at(0)),
                        dynamic_pointer_cast<autodiff::Variable>(domainVariables->at(quantorCount)->at(i)->at(1))});
                poses.push_back(vec);
        ...
        if (ballPose)
        {
            ...
            ballPose =
                    wm->robots->teammates.getTeammatePositionBuffer(executerList->front()).getLastValidContent()->getPoint();
        }
