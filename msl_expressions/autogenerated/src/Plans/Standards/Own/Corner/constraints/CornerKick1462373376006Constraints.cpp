...
void Constraint1464793807994::getConstraint(shared_ptr<ProblemDescriptor> c, shared_ptr<RunningPlan> rp)
{
    ...
    for (int i = 0; i < domainVariables->at(0)->size(); i++)
    {
        ...
        auto vec = make_shared<TVec>(initializer_list<shared_ptr<Term>>{dynamic_pointer_cast<autodiff::Variable>(domainVariables->at(0)->at(i)->at(0)),
    ...
    for (int i = 0; i < domainVariables->at(1)->size(); i++)
    {
        ...
        robotPositions.push_back(robotPos);
        auto vec = make_shared<TVec>(initializer_list<shared_ptr<Term>>{dynamic_pointer_cast<autodiff::Variable>(domainVariables->at(1)->at(i)->at(0)),
    ...
    if (opps != nullptr)
    {
        for (auto opp : *opps)
        {
            ...
            // only add if opp is in close to midline && further away from ball than 1100 (that makes no sense in oppenent standards, because we already use
            // apply rules)
    ...
    if (defender.size() > 0)
    {
        ...
        auto wallPoint = direction * max(ownGoalPos->distanceTo(wm->field->posULOwnPenaltyArea()), ballPose->distanceTo(ownGoalPos) / 2);
        ...
        for (int j = 0; j < defender.size(); j++)
        {
            ...
            util = util + 100 * (1 - (ConstraintBuilder::distanceSqr(defender.at(j), wallPointTVec) / (wm->field->getMaxDistanceSqr())));
        }
    }
    ...
    if (blocker.size() > 0)
    {
        ...
        for (int k = 0; k < blocker.size(); k++)
        {
            ...
            for (int j = 0; j < opponents2BlockCount; j++)
            {
                ...
                //							10
                //									* (1
                //											- ((ConstraintBuilder::distanceSqr(blocker.at(k),
                //blockPositions.at(j)))
                //													/ (wm->field.MaxDistanceSqr *
                //blocker.size()))));

                blockConstraints = blockConstraints | msl::MSLConstraintBuilder::insideSphere(blockPositions.at(j), 200.0, blocker.at(k));
            }
            ...
            util = util + 10 * (1 - ((ConstraintBuilder::distanceSqr(blocker.at(k), ownGoalVec)) / (wm->field->getMaxDistanceSqr())));
