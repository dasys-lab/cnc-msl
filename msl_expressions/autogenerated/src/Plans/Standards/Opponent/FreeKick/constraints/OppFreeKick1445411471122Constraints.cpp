    ...
    void Constraint1445442215438::getConstraint(shared_ptr<ConstraintDescriptor> c, shared_ptr<RunningPlan> rp)
    {
		...
		if(attacker.size()>0) {
			if(ballPose!=nullptr && nearestOpp!=nullptr) {
				...
				for(int j=0; j<attacker.size(); j++) {
					...
					//Near Ball
					utilattack = utilattack + 100*(1-((ConstraintBuilder::distanceSqr(attacker.at(j), tvecBallPose))/(msl::MSLFootballField::MaxDistanceSqr)));
					//Near own goal
					utilattack = utilattack + 10*(1-((ConstraintBuilder::distanceSqr(attacker.at(j), ownGoalVec))/(msl::MSLFootballField::MaxDistanceSqr)));
				}
		...
		if(defender.size()>0) {
			...
			for(int j=0; j<defender.size(); j++) {
				...
				util = util + 100*( 1 - (ConstraintBuilder::distanceSqr(defender.at(j), wallPointTVec)/(msl::MSLFootballField::MaxDistanceSqr)));
