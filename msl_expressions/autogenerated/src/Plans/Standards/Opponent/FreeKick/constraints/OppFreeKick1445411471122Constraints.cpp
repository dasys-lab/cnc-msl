...
namespace alicaAutogenerated
{
    ...
    void Constraint1445442215438::getConstraint(shared_ptr<ConstraintDescriptor> c, shared_ptr<RunningPlan> rp)
    {
    	...
    	constraint = autodiff::LTConstraint::TRUE;

    	auto domainVars = c->getDomainVars();

    	vector<shared_ptr<TVec>> poses;

    	for (int i = 0; i << domainVars->at(0)->size(); i++)
    	{
    		auto test = domainVars->at(0)->at(i);
//    		poses.push_back(make_shared<TVec>(initializer_list<shared_ptr<Term>>{static_cast<Term>(test->at(0)), static_cast<Term>(test->at(1))}));
