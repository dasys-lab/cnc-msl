...
namespace alicaAutogenerated
{
    ...
    void Constraint1445442215438::getConstraint(shared_ptr<ConstraintDescriptor> c, shared_ptr<RunningPlan> rp)
    {
        ...
        shared_ptr < geometry::CNPosition > mopsPose = wm->robots.teammates.getTeamMatePosition(
                (*msl::MSLConstraintBuilder::sc)["Globals"]->get<double>("Globals.Team.mops.ID", NULL));
        if (mopsPose == nullptr)
            mopsPose = make_shared < geometry::CNPosition
                    > (msl::MSLFootballField::posOwnGoalMid()->x, msl::MSLFootballField::posOwnGoalMid()->y, 0);
...
//            if(blocker.size() > 0) util = util + msl::MSLConstraintBuilder::approachUtil(tvecBallPose, blocker);
        ...
        if (mopsPose != nullptr && ballPose != nullptr)
        {
            ...
            shared_ptr < TVec > blockerBallTVec = make_shared < TVec > (initializer_list<double> {blockerBall->x,
                                                                                                  blockerBall->y});
