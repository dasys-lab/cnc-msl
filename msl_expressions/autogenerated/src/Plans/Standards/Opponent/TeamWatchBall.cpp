...
namespace alica
{
    ...
    void TeamWatchBall::run(void* msg)
    {
		...
		if (ballPos != nullptr) {
			...
			if (wm->ball.getVisionBallPositionAndCertaincy()->second<0.5) posV = nullptr;
			if (ballPos!=nullptr && ballPos->length() > 4000) posV = nullptr;
			if (posV!=nullptr) posV = posV - ballPos;
		}
    ...
    void TeamWatchBall::initialiseParameters()
    {
		...
		ballMovedOccurrences = 0;
		auto b1=wm->ball.getVisionBallPosition(0);
		if (b1!= nullptr) ballPos = b1->clone();
		auto b2 = wm->ball.getVisionBallPosition(1);
		...
		if (wm->ball.getVisionBallPositionAndCertaincy()->second<0.4) ballPos = nullptr;

		if (ballPos!=nullptr && ballPos->length() > (wm->game.rules.stayAwayRadius + 750)) ballPos = nullptr;
