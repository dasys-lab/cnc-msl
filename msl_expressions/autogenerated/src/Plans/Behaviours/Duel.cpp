				...
				for (auto obstacle : *mostRecentObs)
				{
					if (obstacle)
					{
						...
						if ((closestObstacle == nullptr
										|| obstacle->distanceTo(ownPoint) < closestObstacle->distanceTo(ownPoint)) && obstacle->distanceTo(ownPoint) > 2000)
						{
...
//								cout << "Duel: distance to friendlyPos: " << friendlyPos->distanceTo(ownPoint) << endl;
			...
			else
			{
				...
				else
				{
					...
					shared_ptr<geometry::CNPoint2D> ballOrth1 = make_shared<geometry::CNPoint2D>(egoBallPos->y,
							-egoBallPos->x);
					shared_ptr<geometry::CNPoint2D> ballOrth2 = make_shared<geometry::CNPoint2D>(-egoBallPos->y,
							egoBallPos->x);
