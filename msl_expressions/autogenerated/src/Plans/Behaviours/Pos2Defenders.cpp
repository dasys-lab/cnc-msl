...
void Pos2Defenders::run(void *msg)
{
    ...
    auto firstDef =
        wm->robots->teammates.getTeamMatePosition(dynamic_cast<const msl::robot::IntRobotID *>((*robotsInOwnEp)[0]));
    msl_actuator_msgs::MotionControl mc;

    if (robotsInOwnEp->size() == 2)
    {

        auto secondDef = wm->robots->teammates.getTeamMatePosition(
            dynamic_cast<const msl::robot::IntRobotID *>((*robotsInOwnEp)[1]));
        ...
        if (firstDef->distanceTo(firstDefPos) < secondDef->distanceTo(firstDefPos))
        {
            ...
            if (ownId == (*robotsInOwnEp)[0])
            {
                ...
                //                                                                  alloBallPos->alloToEgo(*firstDef),
                //                                                                  0,
                //                                                                  additionalPoints);
            ...
            else
            {
                ...
                //                                                                  alloBallPos->alloToEgo(*secondDef),
                //                                                                  0,
                //                                                                  additionalPoints);
