	...
	shared_ptr<geometry::CNPoint2D> ExperimentalRingbuffer::getAvgPoint(int count)
			{
				...
				if (sum_gewichte == 0 ) return nullptr;
...
//			Node.MainNode.RosInfo("Kick it");
		...
		if(wm->rawSensorData.getLastMotionCommand() != nullptr) return;
...
//		long now = supplementary::DateTime::getUtcNowC() / 10000;
		...
		if (ballV3D != nullptr && ballPos != nullptr) {
			...
			if (velo > 1000) {
				...
				double diffTrans = abs(wm->rawSensorData.getLastMotionCommand()->motion.translation-bm_last.motion.translation);
				double diffRot = abs(wm->rawSensorData.getLastMotionCommand()->motion.rotation-bm_last.motion.rotation);
...
//				Point3D ballVelo3DAllo = new Point3D(ballV3DAllo.Vx, ballV3DAllo.Vy, ballV3DAllo.Vz) ;
				shared_ptr<geometry::CNPoint3D> ballVelo3DAllo = make_shared<geometry::CNPoint3D>(ballV3DAllo->x, ballV3DAllo->y, ballV3DAllo->z);
				...
				if (timeBallToGoal > 0) {
					...
					if ( abs(y) < field->GoalWidth/ 2 + 2000) {
						...
						ballVelocity->overWrite(make_shared<geometry::CNPoint2D>(ballV3DAllo->x, ballV3DAllo->y), g);
...
//								Node.MainNode.RosInfo("FIRE EXT 2");
							...
							if (timeBallToGoal2 < 1.0 && abs(dstPointEgo->y - ownPos->y) < 400 && ballInAirTimestamp +3000 > now) {
								km.extension = 1;
								km.extTime = 1000;
								send(km);
