...
void Intercept::run(void *msg)
{
    ...
    {
        predPos->x += cos(od->motion.angle + predPos->theta) * od->motion.translation * this->predictionTimestep / 1000.0;
        predPos->y += sin(od->motion.angle + predPos->theta) * od->motion.translation * this->predictionTimestep / 1000.0;
        predPos->theta += rot;

        predBall->x += egoBallVel->x * this->predictionTimestep / 1000.0;
        predBall->y += egoBallVel->y * this->predictionTimestep / 1000.0;

        if (predBall->distanceTo(predPos) < this->wm->pathPlanner->getRobotRadius() + this->wm->ball->getBallDiameter() / 2) // robotRadius+ballRadius
        {
            break;
        }
    }
    ...
    double controlDist = distErr * this->pdist + this->distIntErr * this->pidist + (distErr - this->lastDistErr) * this->pddist;
    ...
    if (this->wm->field->isInsideField(alloBall, -this->mapInFieldOffset) && !this->wm->field->isInsideField(alloDest))
    {
    ...
    double controlRot = rotErr * this->prot + this->rotIntErr * this->pirot + (rotErr - this->lastRotErr) * this->pdrot;
    ...
    if (egoBallPos->length() < 700 && (currentGameState == msl::GameState::OwnBallPossession || currentGameState == msl::GameState::NobodyInBallPossession))
    {
        controlRot *= 2.3;
...
void Intercept::initialiseParameters()
{
    ...
    if (getParameter("StandardSituationReceiver", tmp))
    {
        ...
        {
            predictionHorizon = (*this->sc)["Drive"]->get<int>("Drive.Intercept.StandardSituation.predictionHorizon", NULL);
        }
    }
