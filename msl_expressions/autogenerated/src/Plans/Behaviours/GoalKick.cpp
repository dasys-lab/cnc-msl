...
namespace alica
{
	...
	void GoalKick::run(void* msg)
	{
		...
		{
			if (!leftBlocked && wm->pathPlanner.corridorCheck(vNet, make_shared<geometry::CNPoint2D>(ownPos->x, ownPos->y), leftAimPoint,
												make_shared<geometry::CNPoint2D>(obs->at(i).x, obs->at(i).y)))
			{
				leftBlocked = true;
			}
			if (!midBlocked && wm->pathPlanner.corridorCheck(vNet, make_shared<geometry::CNPoint2D>(ownPos->x, ownPos->y), midAimPoint,
												make_shared<geometry::CNPoint2D>(obs->at(i).x, obs->at(i).y)))
			{
				midBlocked = true;
			}
			if (!rightBlocked && wm->pathPlanner.corridorCheck(vNet, make_shared<geometry::CNPoint2D>(ownPos->x, ownPos->y), rightAimPoint,
												make_shared<geometry::CNPoint2D>(obs->at(i).x, obs->at(i).y)))
			{
				rightBlocked = true;
			}
	...
	void GoalKick::initialiseParameters()
	{
		...
		leftAimPoint = make_shared<geometry::CNPoint2D>(field->FieldLength + 250,field->posLeftOppGoalPost()->y - wm->ball.getBallDiameter() * 1.5);
		midAimPoint = make_shared<geometry::CNPoint2D>(field->FieldLength + 250, 0);
		rightAimPoint = make_shared<geometry::CNPoint2D>(field->FieldLength + 250,field->posRightOppGoalPost()->y + wm->ball.getBallDiameter() * 1.5);
		aimPoint = nullptr;
		angleTolerance = 0.05;
		minKickPower = 1500.0;
		/*PROTECTED REGION END*/
