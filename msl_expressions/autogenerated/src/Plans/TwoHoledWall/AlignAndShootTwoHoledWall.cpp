...
namespace alica
{
            ...
            DomainBehaviour("AlignAndShootTwoHoledBall")
    {
		...
		auto lowKickSectinList((*this->sc)["Show"]->getSections("TwoHoledWall", "LowKickList", NULL));

		lowKickList.reserve(lowKickSectinList->size());

		auto highKickSectinList((*this->sc)["Show"]->getSections("TwoHoledWall", "HighKickList", NULL));

		highKickList.reserve(highKickSectinList->size());
		try
		{
			int i=1;
			while( true )
			{
				...
				for(int j=0; j<lowKickSectinList->size(); j++) {
					if(lowKickSectinList->at(j) == currentSearchString) {
						found = true;
						break;
					}
				}
			...
			while( true )
			{
				bool found = false;
				string currentSearchString = string("p", i);
				for(int j=0; j<highKickSectinList->size(); j++) {
					if(highKickSectinList->at(j) == currentSearchString) {
						found = true;
						break;
					}
				}
...
//		}
		...
		if(timesOnTarget>TIMES_ON_TARGET && fabs(this->voltage4shoot-voltage) < 1.001 ) {
			...
			this->success = true;
			cout << "Dist: " << egoTarget->length() << "\tPower: " << kc.power << "\tDeviation: " << sin(deltaAngle)*egoTarget->length() << ",\tVolt: " << volt << endl;

		} else {
			mc.motion.rotation = (mc.motion.rotation<0?-1:1)*min(this->maxRot, max(fabs(mc.motion.rotation),this->minRot));
		}
...
/*PROTECTED REGION ID(methods1417620683982) ENABLED START*/ //Add additional methods here
    ...
    double AlignAndShootTwoHoledWall::interPolatePower(double dist,
    		vector<shared_ptr<CNPoint2D> > values) {
    	int i=0;
		while(i < values.size() && dist > values[i]->x) i++;
		if (i==0) return values[0]->y;
		if (i==values.size()) return values[values.size()-1]->y;
		return values[i-1]->y + (dist - values[i-1]->x)/(values[i]->x-values[i-1]->x)*(values[i]->y-values[i-1]->y);
    }
