    ...
    : DomainBehaviour("Parking")
{
    ...
    this->distanceToParkingPositionTolerance =
        (*this->sc)["Parking"]->get<double>("ParkingPositions", "distanceToParkingPositionTolerance", NULL);
    this->rm = new msl::RobotMovement();
    this->movementQuery = make_shared<msl::MovementQuery>();
    this->parkingPosition =
        make_shared<geometry::CNPoint2D>(this->parkingSlotIdx * -this->offset, wm->field->getFieldWidth() / 2.0);
    /*PROTECTED REGION END*/
}
Parking::~Parking()
{
...
void Parking::initialiseParameters()
{
    ...
    this->parkingSlotIdx = (*this->sc)["Parking"]->get<double>(
        "ParkingPositions", to_string(dynamic_cast<const msl::robot::IntRobotID*>(this->getOwnId())->getId()).c_str(),
        NULL);
    this->parkingPosition =
        make_shared<geometry::CNPoint2D>(this->parkingSlotIdx * -this->offset, wm->field->getFieldWidth() / 2.0);
    /*PROTECTED REGION END*/
