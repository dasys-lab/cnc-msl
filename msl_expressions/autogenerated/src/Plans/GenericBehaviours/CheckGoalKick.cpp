...
void CheckGoalKick::initialiseParameters() {
	...
	alloLeftAimPoint = make_shared<geometry::CNPoint2D>(
			field->FieldLength / 2 + 250,
			field->posLeftOppGoalPost()->y - wm->ball.getBallDiameter() * 1.5);
	alloMidAimPoint = make_shared<geometry::CNPoint2D>(
			field->FieldLength / 2 + 250, 0);
	alloRightAimPoint = make_shared<geometry::CNPoint2D>(
			field->FieldLength / 2 + 250,
			field->posRightOppGoalPost()->y + wm->ball.getBallDiameter() * 1.5);
	/*PROTECTED REGION END*/
}
...
//
	...
	cout << "if condition1: "
			<< (egoTarget->angleTo() > (M_PI - toleranceAngle)) << endl;
	cout << "if condition2: "
			<< (egoTarget->angleTo() > (-M_PI + toleranceAngle)) << endl;

	// if angle is smaller then tolerance angle return true
	if (egoTarget->angleTo() > (M_PI - toleranceAngle)
			|| egoTarget->angleTo() < (-M_PI + toleranceAngle)) {
...
bool CheckGoalKick::checkShootPossibility() {
	cout
			<< "checkShootPossibility() ============================================"
			<< endl;
	// check if obstacle lays in corridor
	auto obstacles = wm->robots.getObstacles();
	shared_ptr<geometry::CNPoint2D> obstaclePoint = make_shared<
			geometry::CNPoint2D>(0, 0);
	...
	for (int i = 0; i < obstacles->size(); i++) {
		...
		if (wm->pathPlanner.corridorCheck(
				wm->pathPlanner.getCurrentVoronoiNet(), ownPos, goalPosMiddle,
				obstaclePoint)) {
			foundObstacle = true;
			obstacleAt = i;
			break;
	...
	if (obstacleAt > -1) {
		...
		shared_ptr<geometry::CNPoint2D> obstaclePos = make_shared<
				geometry::CNPoint2D>(0, 0);
		obstaclePos->x = obstacle.x;
		obstaclePos->y = obstacle.y;

		cout << "ownPos <----> obstaclePos = "
				<< ownPos->distanceTo(obstaclePos) << endl;
		cout << "if condition 1 = "
				<< (ownPos->distanceTo(obstaclePos) < robotShootDistanceOwn)
				<< endl;
		cout << "obstaclePos <----> GoalPosMiddle = "
				<< obstaclePos->distanceTo(goalPosMiddle) << endl;
		cout << "if condition 2 = "
				<< (obstaclePos->distanceTo(goalPosMiddle)
						< robotShootDistanceGoal) << endl;

		if (ownPos->distanceTo(obstaclePos) < robotShootDistanceOwn
				|| obstaclePos->distanceTo(goalPosMiddle)
						< robotShootDistanceGoal) {
			return false;
		} else {
			return true;
		}
...
void CheckGoalKick::readConfigParameters() {
	...
	robotShootDistanceOwn = (*sc)["GoalKick"]->get<double>(
			"GoalKick.Default.robotShootDistanceOwn", NULL);
	robotShootDistanceGoal = (*sc)["GoalKick"]->get<double>(
			"GoalKick.Default.robotShootDistanceGoal", NULL);
	minKickPower = (*sc)["GoalKick"]->get<double>(
			"GoalKick.Default.minKickPower", NULL);
	wheelSpeedLeft = (*sc)["GoalKick"]->get<double>(
			"GoalKick.Default.wheelSpeedLeft", NULL);
	wheelSpeedRight = (*sc)["GoalKick"]->get<double>(
			"GoalKick.Default.wheelSpeedRight", NULL);
}
...
//		send(bhc);
	...
	for (int i = 0; i < obs->size(); i++) {
		...
		if (wm->pathPlanner.corridorCheckBall(vNet,
				make_shared<geometry::CNPoint2D>(ownPos->x, ownPos->y),
				alloLeftAimPoint,
				make_shared<geometry::CNPoint2D>(obs->at(i).x, obs->at(i).y)->egoToAllo(
						*ownPosV))) {
			leftBlocked = true;
		}
		if (wm->pathPlanner.corridorCheckBall(vNet,
