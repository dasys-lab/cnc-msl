...
void AlignAndPassRapid::run(void *msg)
{
    ...
    if (egoBallPos == nullptr)
    {
        ...
        this->logger->log("AlignAndPassRapid", "Ego Ball is null", msl::LogLevels::error);
        return;
    }
    ...
    if (alloBall == nullptr)
    {
        ...
        this->logger->log("AlignAndPassRapid", "Allo Ball is null", msl::LogLevels::error);
        return;
    }
    ...
    if (eps.size() <= 0)
    {
        ...
        this->logger->log("AlignAndPassRapid", "All EPs is null", msl::LogLevels::error);
        return;
    }
    ...
    for (EntryPoint *ep : eps)
    {
        ...
        if (teammates == nullptr)
        {
            ...
            this->logger->log("AlignAndPassRapid", "No Teammate for entry point " + ep->toString(), msl::LogLevels::error);
            return;
        }
    ...
    if (teamMateIds.size() <= 0)
    {
        ...
        this->logger->log("AlignAndPassRapid", "Something Strange is going on with RobotIDs and Entrypoints", msl::LogLevels::error);
        return;
    }
    ...
    if (vNet == nullptr)
    {
        ...
        this->logger->log("AlignAndPassRapid", "VNet is null!", msl::LogLevels::error);
        return;
    }
    ...
    if (matePoses == nullptr)
    {
        ...
        this->logger->log("AlignAndPassRapid", "matePos == nullptr", msl::LogLevels::error);
        return;
    }
    ...
    { // +/-5 degree
        ...
        shared_ptr<geometry::CNPoint2D> goalReceiverVec = dest - make_shared<geometry::CNPoint2D>(bestAoc->x, bestAoc->y);
        double v0 = 0;
...
void AlignAndPassRapid::findBestPassPoint(double cf, shared_ptr<geometry::CNPoint2D> passPoint, shared_ptr<geometry::CNPoint2D> receiver,
                                          shared_ptr<msl::VoronoiNet> vNet, shared_ptr<geometry::CNPosition> teamMatePos, int teamMateId)
{
        ...
        )
    {
        ...
        if (geometry::absDeltaAngle(alloPos->theta + M_PI, (passPoint - make_shared<geometry::CNPoint2D>(alloPos->x, alloPos->y))->angleTo()) > maxTurnAngle)
        {
#ifdef DBM_DEBUG
...
bool AlignAndPassRapid::outsideCorridore(shared_ptr<geometry::CNPoint2D> ball, shared_ptr<geometry::CNPoint2D> passPoint, double passCorridorWidth,
...
bool AlignAndPassRapid::outsideCorridoreTeammates(shared_ptr<geometry::CNPoint2D> ball, shared_ptr<geometry::CNPoint2D> passPoint, double passCorridorWidth,
                                                  shared_ptr<vector<shared_ptr<geometry::CNPoint2D>>> points)
{
    for (int i = 0; i < points->size(); i++)
    {
...
bool AlignAndPassRapid::outsideTriangle(shared_ptr<geometry::CNPoint2D> a, shared_ptr<geometry::CNPoint2D> b, shared_ptr<geometry::CNPoint2D> c,
                                        double tolerance, shared_ptr<vector<shared_ptr<geometry::CNPoint2D>>> points)
{
