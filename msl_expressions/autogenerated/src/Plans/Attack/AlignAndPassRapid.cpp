...
void AlignAndPassRapid::run(void *msg)
{
    ...
    dbm->topic = "Pass";
#endif
    for (auto teamMateId : this->teamMateIds)
    {
        shared_ptr<vector<shared_ptr<geometry::CNPoint2D>>> vertices =
            vNet->getTeamMateVerticesCNPoint2D(dynamic_cast<const msl::robot::IntRobotID *>(teamMateId));
        shared_ptr<geometry::CNPosition> teamMatePos = wm->robots->teammates.getTeamMatePosition(dynamic_cast<const msl::robot::IntRobotID *>(teamMateId));
    ...
    {   // +/-5 degree
        ...
        pinf.y = alloPos->y;
        pm.origin = pinf;
        pm.receiverID.id = dynamic_cast<const msl::robot::IntRobotID*>(bestTeamMateId)->toByteVector();
...
void AlignAndPassRapid::findBestPassPoint(double cf, shared_ptr<geometry::CNPoint2D> passPoint, shared_ptr<geometry::CNPoint2D> receiver,
                                          shared_ptr<msl::VoronoiNet> vNet, shared_ptr<geometry::CNPosition> teamMatePos, const supplementary::IAgentID* teamMateId)
{
