...
namespace alicaAutogenerated
{
    ...
    bool TransitionCondition1450175866027::evaluate(shared_ptr<RunningPlan> rp)
    {
		...
		if (abs(ball->y) < msl::MSLFootballField::FieldWidth/2-threshold || abs(ball->x) < msl::MSLFootballField::FieldLength/2-threshold) return false;
    ...
    bool TransitionCondition1450175867019::evaluate(shared_ptr<RunningPlan> rp)
    {
    	...
    	if(rp->anyChildrenStatus(PlanStatus::Failed)) return true;
    	double threshold = 500;
		auto ball = wm->ball.getAlloBallPosition();
		if (ball==nullptr) return true;
		if (abs(ball->y) < msl::MSLFootballField::FieldWidth/2-threshold || abs(ball->x) < msl::MSLFootballField::FieldLength/2-threshold) return true;
        /*PROTECTED REGION END*/
    ...
    bool TransitionCondition1450175926118::evaluate(shared_ptr<RunningPlan> rp)
    {
		...
		if(egoBall == nullptr) return true;
		double ballDistance = egoBall->length();
		return (isTimeOut(150*1000000, rp->getStateStartTime(), rp) && ballDistance < expectedBallTravelDistance / 2.0) || isTimeOut(1500*1000000, rp->getStateStartTime(), rp);
