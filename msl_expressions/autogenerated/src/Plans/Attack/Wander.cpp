...
namespace alica
{
			...
			DomainBehaviour("Wander")
	{
		...
		targetPoints[0] = make_shared<geometry::CNPoint2D>(-(fieldLength / 2 - distToCorner),
															fieldWidth / 2 - distToCorner);
		targetPoints[1] = make_shared<geometry::CNPoint2D>(-(fieldLength / 2 - distToCorner),
															-(fieldWidth / 2 - distToCorner));
		targetPoints[2] = make_shared<geometry::CNPoint2D>(0.0, 0.0);
		targetPoints[3] = make_shared<geometry::CNPoint2D>(fieldLength / 2 - distToCorner,
															fieldWidth / 2 - distToCorner);
		targetPoints[4] = make_shared<geometry::CNPoint2D>(fieldLength / 2 - distToCorner,
															-(fieldWidth / 2 - distToCorner));

		//drive into both opps corners
		targetPointsOwnCorner.resize(2);
		targetPointsOwnCorner[0] = make_shared<geometry::CNPoint2D>(fieldLength / 2 - distToCorner,
																	fieldWidth / 2 - distToCorner);
		targetPointsOwnCorner[1] = make_shared<geometry::CNPoint2D>(fieldLength / 2 - distToCorner,
																	-(fieldWidth / 2 - distToCorner));

		//drive into both own corners
		targetPointsOppCorner.resize(2);
		targetPointsOppCorner[0] = make_shared<geometry::CNPoint2D>(-(fieldLength / 2 - distToCorner),
																	fieldWidth / 2 - distToCorner);
		targetPointsOppCorner[1] = make_shared<geometry::CNPoint2D>(-(fieldLength / 2 - distToCorner),
																	-(fieldWidth / 2 - distToCorner));

		//drive to both own penaltyarea corners
		targetPointsOwnGoalKick.resize(2);
		targetPointsOwnGoalKick[0] = make_shared<geometry::CNPoint2D>(-(fieldLength / 2 - field->GoalInnerAreaWidth),
																		field->GoalInnerAreaLength / 2);
		targetPointsOwnGoalKick[1] = make_shared<geometry::CNPoint2D>(-(fieldLength / 2 - field->GoalInnerAreaWidth),
																		-field->GoalInnerAreaLength / 2);

		//drive to both opp penaltyarea corners
		targetPointsOppGoalKick.resize(2);
		targetPointsOppGoalKick[0] = make_shared<geometry::CNPoint2D>(fieldLength / 2 - field->GoalInnerAreaWidth,
																		field->GoalInnerAreaLength / 2);
		targetPointsOppGoalKick[1] = make_shared<geometry::CNPoint2D>(fieldLength / 2 - field->GoalInnerAreaWidth,
																		-field->GoalInnerAreaLength / 2);
		...
		//right
		targetPointsThrowIn[3] = make_shared<geometry::CNPoint2D>(fieldLength / 4, -(fieldWidth / 2 - distToOutLine));
		targetPointsThrowIn[4] = make_shared<geometry::CNPoint2D>(0.0, -(fieldWidth / 2 - distToOutLine));
		targetPointsThrowIn[5] = make_shared<geometry::CNPoint2D>(-fieldLength / 4, -(fieldWidth / 2 - distToOutLine));
		/*PROTECTED REGION END*/
	}
	Wander::~Wander()
	{
