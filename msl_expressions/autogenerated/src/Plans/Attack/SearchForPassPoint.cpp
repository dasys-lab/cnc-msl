			...
			for (int teamMateId : this->teamMateIds)
			{
				...
				for (int i = 0; i < vertices->size(); i++)
				{
					...
					if (ff->isInsideField (passPoint, distToFieldBorder) // pass point must be inside the field with distance to side line of 1.5 metre
							&& !ff->isInsidePenalty(passPoint, 0.0)
...
//						}
						...
						if (geometry::GeometryCalculator::absDeltaAngle(alloPos->theta + M_PI, (passPoint - make_shared<geometry::CNPoint2D>(alloPos->x, alloPos->y))->angleTo()) > maxTurnAngle)
						{
							continue;
						}
						...
						shared_ptr<geometry::CNPoint2D> ball2PassPointOrth = make_shared<geometry::CNPoint2D>(-ball2PassPoint->y, ball2PassPoint->x)->normalize() * ratio * passLength;
