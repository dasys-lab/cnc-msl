...
//        cout << "Attackplay: Constraint called" << endl;
        ...
        nonstd::optional<geometry::CNPositionAllo> passerPos = nonstd::nullopt;
        if (r1->size() == 0 || r1 == nullptr)
        {
            ...
            passerPos = nonstd::make_optional<geometry::CNPositionAllo>();
        }
        else
        {
            for (int id : *r1)
            {
                passerPos = wm->robots->teammates.getTeammatePositionBuffer(id).getLastValidContent();
                if (passerPos)
                {
        ...
        else
        {
            for (int id : *r2)
            {
                auto p = wm->robots->teammates.getTeammatePositionBuffer(id).getLastValidContent();
                if (!p )
                {
...
//        }
        ...
        util = util + 100000 - (make_shared<autodiff::Abs>(targetDist - ConstraintBuilder::distance(target, passer)))
                + minOppDist;
        c->setConstraint(dynamic_pointer_cast<alica::SolverTerm>(constraint));
        c->setUtility(dynamic_pointer_cast<alica::SolverTerm>(util));
        /*PROTECTED REGION END*/
