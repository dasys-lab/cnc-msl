...
namespace alica
{
	...
	void DribbleToAttackPoint::run(void* msg)
	{
		...
		{
			auto weightedOppVector = closestOpponent->normalize()->rotate(M_PI) * 1000 * (1.0 / closestOpponent->length());
			auto weightedTargetVector = egoTargetPoint->normalize()->rotate(M_PI) * 1000 * (1.0 / egoTargetPoint->length());
			egoAlignPoint = (weightedOppVector + weightedTargetVector)->normalize() * 1000;
		}
		msl_actuator_msgs::MotionControl mc = msl::RobotMovement::moveToPointCarefully(egoTargetPoint, egoAlignPoint, 250);
