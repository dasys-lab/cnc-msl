...
//        double rawBallConf = wm->ball->getBallConfidence
//        auto rawBall = wm->ball->getPositionEgo();
//        bool rawBallChecked = rawBall && rawBallConf > 0.4;
        auto rawBall = wm->ball->getVisionBallPositionBuffer().getLastValidContent();
        bool rawBallChecked = rawBall.has_value();
        if (!wm->ball->getPositionEgo())
        {
            return rawBallChecked;
        }
        else
        {
            return rawBallChecked && wm->ball->getPositionEgo()->length() < 3000.0;
        }
        /*PROTECTED REGION END*/
    ...
    bool TransitionCondition1436536104021::evaluate(shared_ptr<RunningPlan> rp)
    {
        ...
        return !wm->whiteBoard->getPassMsgBuffer().getLastValidContent() && wm->game->getGameState() == msl::GameState::OwnBallPossession;
        /*PROTECTED REGION END*/
    ...
    bool TransitionCondition1436536151584::evaluate(shared_ptr<RunningPlan> rp)
    {
        ...
        //TODO is this accurately describing the ball confidence in nice geom?
        auto rawBall = wm->ball->getVisionBallPositionBuffer().getLastValidContent();
        auto rawBallInfo = wm->ball->getVisionBallPositionBuffer().getLastValid();
        return rawBall && rawBallInfo->getCertainty() > 0.4;
        /*PROTECTED REGION END*/
