...
namespace alica
{
			...
			DomainBehaviour("StandardAlignToPoint2Receivers")
	{
		...
		this->receiverBallMovedThreshold = (*this->sc)["StandardSituation"]->get<double>(
				"StandardAlignToPoint", "receiverBallMovedThreshold",
				NULL);
	...
	void StandardAlignToPoint2Receivers::run(void* msg)
	{
		...
		cout << fabs(receiverPos->alloToEgo(*ownPos)->rotate(M_PI)->angleTo()) << " < " << this->alignAngleTolerance
				<< endl;

		if (egoTarget->length() < this->positionDistanceTolerance
				&& fabs(receiverPos->alloToEgo(*ownPos)->rotate(M_PI)->angleTo()) < this->alignAngleTolerance)
		{
