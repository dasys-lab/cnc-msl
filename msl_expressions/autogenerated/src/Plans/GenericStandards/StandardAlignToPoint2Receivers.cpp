...
void StandardAlignToPoint2Receivers::run(void *msg)
{
    ...
    if (!wm->field->isInsidePenalty(passPoint, 0.0))
    {
        ...
        if (canPass && !outsideTriangle(alloBall, right, left, ballRadius, obs) && !outsideCorridore(alloBall, passPoint, this->passCorridorWidth, obs))
        {
            canPass = false;
        }
    }
...
bool StandardAlignToPoint2Receivers::outsideTriangle(shared_ptr<geometry::CNPoint2D> a, shared_ptr<geometry::CNPoint2D> b, shared_ptr<geometry::CNPoint2D> c,
