...
namespace alica
{
	...
	void StandardAlignAndGrab2Receivers::run(void *msg)
	{
		...
		{
			passPoint = make_shared<geometry::CNPoint2D>(this->recPos->x,
															-this->wm->field->getFieldWidth() / 2.0 + 1000.0);
		}
		else
		{
			passPoint = make_shared<geometry::CNPoint2D>(this->recPos->x,
...
//            cout << "not have ball else if" << endl;
		...
		mc.motion.rotation = -fac * rot
				* (2 * fabs(0.8 * dangle + 0.1 * this->angleIntErr + 2 * (dangle - this->oldAngleErr)));
					...
					|| (haveBallCounter > 3
							&& ((runningTimeMS <= 3000.0 && fabs(dangle) < this->minTol)
									|| fabs(dangle)
											< this->minTol
													+ max(0.0,
															(this->tol - this->minTol)
																	/ (3000.0 / (runningTimeMS - 3000.0))))))
			{
				mc.motion.angle = M_PI;
