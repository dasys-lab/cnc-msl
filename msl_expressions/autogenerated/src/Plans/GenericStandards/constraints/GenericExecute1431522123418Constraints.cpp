...
namespace alicaAutogenerated
{
    ...
    void Constraint1457955744730::getConstraint(shared_ptr<ProblemDescriptor> c, shared_ptr<RunningPlan> rp)
    {
        ...
        for (int i = 0; i < domainVariables->at(2)->size(); i++)
        {
            ...
            auto robotPos = wm->robots->teammates.getTeamMatePosition(dynamic_cast<const msl::robot::IntRobotID*>(c->getAgentsInScope()->at(2)->at(i)));
            if (!robotPos)
            {
                //safety measure in case one agent crashes
                continue;
        ...
        auto r1 = rp->getAssignment()->getRobotsWorking(1431522269326);
        ...
        else
        {
            for (auto id : *r1)
            {
                futurePasserPos = wm->robots->teammates.getTeamMatePosition(dynamic_cast<const msl::robot::IntRobotID*>(id));
