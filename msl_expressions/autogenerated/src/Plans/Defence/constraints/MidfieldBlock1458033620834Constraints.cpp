...
namespace alicaAutogenerated
{
	...
	void Constraint1458033723845::getConstraint(shared_ptr<ConstraintDescriptor> c, shared_ptr<RunningPlan> rp)
	{
		...
		c->getStaticRanges()->at(0).at(0) = -field->FieldLength / 2.0;
		c->getStaticRanges()->at(0).at(1) = field->FieldLength / 2.0;
		c->getStaticRanges()->at(1).at(0) = -field->FieldWidth / 2.0;
		c->getStaticRanges()->at(1).at(1) = field->FieldWidth / 2.0;
		...
		shared_ptr<autodiff::TVec> tAttackerPos = make_shared<autodiff::TVec>(
				initializer_list<double> {attackerPos->x, attackerPos->y});
		...
		if (ownPos != nullptr && ball != nullptr)
		{
			...
			constraint = constraint
					& ConstraintBuilder::distanceSqr(p1, tAttackerPos)
							< autodiff::TermBuilder::constant(5500.0 * 5500.0);
			...
			shared_ptr<geometry::CNPoint2D> attacker2ballOrth =
					make_shared<geometry::CNPoint2D>(-attacker2ball->y, attacker2ball->x)->normalize() * ratio
							* freeWayLength;
			...
			util = util + (1 - ConstraintBuilder::distance(p1, talloBall) / field->MaxDistance);
				...
				for (int i = 0; i < opps->size(); i++)
				{
					...
					op1 = op1
							| (ConstraintBuilder::equals(p1, tbpos, 10)
									& (msl::MSLConstraintBuilder::outsideTriangle(tAttackerPos, tRight, tLeft, 500.0,
																					v)
							...
							* (1
									- (ConstraintBuilder::distanceSqr(
											...
											make_shared<autodiff::TVec>(
													initializer_list<double> {ownPos->x, ownPos->y}))
											/ (field->MaxDistance * field->MaxDistance)));
