...
void Constraint1458033723845::getConstraint(shared_ptr<ProblemDescriptor> c, shared_ptr<RunningPlan> rp)
{
    ...
    {
        attackerPos = wm->robots->teammates.getTeammatePositionBuffer(attacker->at(0)).getLastValidContent();
    }
    ...
    shared_ptr<autodiff::TVec> tAttackerPos =
        make_shared<autodiff::TVec>(initializer_list<double>{attackerPos->x, attackerPos->y});
    ...
    auto alloBall = ball->toAllo(*ownPos);

    shared_ptr<autodiff::TVec> talloBall =
        make_shared<autodiff::TVec>(initializer_list<double>{alloBall.x, alloBall.y});
    // min max distance
    constraint =
        constraint & ConstraintBuilder::distanceSqr(p1, talloBall) > autodiff::TermBuilder::constant(400.0 * 400.0);
    ...
    geometry::CNVecAllo attacker2ballOrth =
        geometry::CNVecAllo(-attacker2ball.y, attacker2ball.x).normalize() * ratio * freeWayLength;

    geometry::CNPointAllo left = freeWayPoint + attacker2ballOrth;
    shared_ptr<autodiff::TVec> tLeft = make_shared<autodiff::TVec>(initializer_list<double>{left.x, left.y});
    geometry::CNPointAllo right = freeWayPoint - attacker2ballOrth;
    ...
    geometry::CNVecAllo attacker2Own = ownPos->getPoint() - attackerPos->getPoint();
    ...
    if (distToCake > 300)
    {
        ...
        v.push_back(p1);
        constraint =
            constraint & msl::MSLConstraintBuilder::outsideCakePiece(tAttackerPos, talloBall, town2Attacker, 150, v);
    }
    else if (distToCake < -300)
    {
        ...
        v.push_back(p1);
        constraint =
            constraint & msl::MSLConstraintBuilder::outsideCakePiece(tAttackerPos, town2Attacker, talloBall, 150, v);
    } // else no piece of cake for us :(

    shared_ptr<autodiff::Term> op1 = autodiff::Term::FALSE;
    auto opps = wm->robots->opponents.getOpponentsAlloClusteredBuffer().getLastValidContent(); // GetTrackedOpponents();
    if (opps->size() > 0)
    {
        for (int i = 0; i < opps->size(); i++)
        {
            geometry::CNPointAllo bpos = opps->at(i) + (alloBall - opps->at(i)).normalize() * 600;
            ...
            vector<shared_ptr<TVec>> v;
            v.push_back(tbpos);
            op1 = op1 | (ConstraintBuilder::equals(p1, tbpos, 10) &
    ...
    if (mates)
    {
        for (int i = 0; i < mates->size(); i++)
        {
            ...
            constraint = constraint &
                         ConstraintBuilder::distanceSqr(p1, mateVec[i]) >
    ...
    if (ballPos &&
        (wm->whiteBoard->getPassMsgBuffer().getLastValidContent() || wm->whiteBoard->getPassMsgBuffer().getLastValidContent()->receiverID != wm->getOwnId()))
    {
    ...
    if (ownPos != nullptr)
    {
        ...
        util = util +
               0.75 * (1 - (ConstraintBuilder::distanceSqr(
                                p1, make_shared<autodiff::TVec>(initializer_list<double>{ownPos->x, ownPos->y})) /
