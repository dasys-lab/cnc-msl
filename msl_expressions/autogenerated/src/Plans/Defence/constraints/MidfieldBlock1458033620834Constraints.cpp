...
void Constraint1458033723845::getConstraint(shared_ptr<ProblemDescriptor> c, shared_ptr<RunningPlan> rp)
{
    ...
    {
        attackerPos = wm->robots->teammates.getTeamMatePosition(dynamic_cast<const msl::robot::IntRobotID *>(attacker->at(0)));
    }
    ...
    shared_ptr<autodiff::TVec> talloBall = make_shared<autodiff::TVec>(initializer_list<double>{alloBall->x, alloBall->y});
    // min max distance
    constraint = constraint & ConstraintBuilder::distanceSqr(p1, talloBall) > autodiff::TermBuilder::constant(400.0 * 400.0);
    constraint = constraint & ConstraintBuilder::distanceSqr(p1, tAttackerPos) < autodiff::TermBuilder::constant(5500.0 * 5500.0);

    shared_ptr<geometry::CNPoint2D> attacker2ball = alloBall - attackerPos;
    double freeWayLength = 1500.0;
    shared_ptr<geometry::CNPoint2D> freeWayPoint = attackerPos->getPoint() + attacker2ball->normalize() * freeWayLength;
    double ratio = tan(45 * M_PI / 180);
    shared_ptr<geometry::CNPoint2D> attacker2ballOrth =
        make_shared<geometry::CNPoint2D>(-attacker2ball->y, attacker2ball->x)->normalize() * ratio * freeWayLength;

    shared_ptr<geometry::CNPoint2D> left = freeWayPoint + attacker2ballOrth;
    shared_ptr<autodiff::TVec> tLeft = make_shared<autodiff::TVec>(initializer_list<double>{left->x, left->y});
    shared_ptr<geometry::CNPoint2D> right = freeWayPoint - attacker2ballOrth;
    shared_ptr<autodiff::TVec> tRight = make_shared<autodiff::TVec>(initializer_list<double>{right->x, right->y});

    util = util + (1 - ConstraintBuilder::distance(p1, talloBall) / wm->field->getMaxDistance());

    shared_ptr<geometry::CNPoint2D> own2Attacker = attackerPos->getPoint() - ownPos->getPoint() + attackerPos->getPoint();
    shared_ptr<autodiff::TVec> town2Attacker = make_shared<autodiff::TVec>(initializer_list<double>{own2Attacker->x, own2Attacker->y});

    shared_ptr<geometry::CNPoint2D> attacker2Own = ownPos->getPoint() - attackerPos->getPoint();
    double distToCake = (attacker2Own->x * attacker2ball->y - attacker2Own->y * attacker2ball->x) / attacker2Own->length();
    ...
    if (distToCake > 300)
    {
        ...
        v.push_back(p1);
        constraint = constraint & msl::MSLConstraintBuilder::outsideCakePiece(tAttackerPos, talloBall, town2Attacker, 150, v);
    }
    else if (distToCake < -300)
    {
        ...
        vector<shared_ptr<TVec>> v;
        v.push_back(p1);
        constraint = constraint & msl::MSLConstraintBuilder::outsideCakePiece(tAttackerPos, town2Attacker, talloBall, 150, v);
    ...
    if (opps->size() > 0)
    {
        for (int i = 0; i < opps->size(); i++)
        {
            ...
            shared_ptr<autodiff::TVec> tbpos = make_shared<autodiff::TVec>(initializer_list<double>{bpos->x, bpos->y});
            // Equality constraint is more efficient than 'distance < x' constraint, use if a very small area is ment
            vector<shared_ptr<TVec>> v;
            v.push_back(tbpos);
            op1 =
                op1 | (ConstraintBuilder::equals(p1, tbpos, 10) & (msl::MSLConstraintBuilder::outsideTriangle(tAttackerPos, tRight, tLeft, 500.0, v) |
                                                                   ConstraintBuilder::distanceSqr(tbpos, p1) < autodiff::TermBuilder::constant(300.0 * 300.0)));
        }
    }
    ...
    if (mates != nullptr)
    {
        for (int i = 0; i < mates->size(); i++)
        {
            ...
            constraint = constraint & ConstraintBuilder::distanceSqr(p1, mateVec[i]) > autodiff::TermBuilder::constant(750 * 750); // get away from mates
        }
    }
                                      ...
                                      wm->getOwnId()->toByteVector().begin())))
    {
        ...
        constraint = constraint & (ConstraintBuilder::distanceSqr(p1, tvecBallPose) > autodiff::TermBuilder::constant(1500 * 1500));
    }
    ...
    if (ownPos != nullptr)
    {
        ...
        util = util +
               0.75 * (1 - (ConstraintBuilder::distanceSqr(p1, make_shared<autodiff::TVec>(initializer_list<double>{ownPos->x, ownPos->y})) /
