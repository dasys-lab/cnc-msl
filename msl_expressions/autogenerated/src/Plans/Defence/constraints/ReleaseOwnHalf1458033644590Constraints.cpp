...
namespace alicaAutogenerated
{
    ...
    void Constraint1458033759784::getConstraint(shared_ptr<ProblemDescriptor> c, shared_ptr<RunningPlan> rp)
    {
        ...
        if (attacker != nullptr)
        {
            ...
            {
                attackerPos = wm->robots->teammates.getTeammatePositionBuffer(mateId).getLastValidContent();
                break;
            }
        }
        ...
        auto teammates = wm->robots->teammates.getTeammatesAlloClusteredBuffer().getLastValidContent();
        if (teammates->size() > 0)
        {
        ...
        auto pos = wm->rawSensorData->getOwnPositionVisionBuffer().getLastValidContent();
        if (!pos)
        {
            geometry::CNVecAllo distVec1 = (attackerPos->getPoint() + geometry::CNVecAllo(-400.0, 2000)) - pos->getPoint();
            geometry::CNVecAllo distVec2 = (attackerPos->getPoint() + geometry::CNVecAllo(-400.0, -2000)) - pos->getPoint();
            ...
            if (ballPos
                    && (!wm->whiteBoard->getPassMsgBuffer().getLastValidContent()
                            || wm->whiteBoard->getPassMsgBuffer().getLastValidContent()->receiverID != wm->getOwnId()))
            {
