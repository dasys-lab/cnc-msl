    ...
    void Constraint1458033759784::getConstraint(shared_ptr<ConstraintDescriptor> c, shared_ptr<RunningPlan> rp)
    {
        ...
        if (pos != nullptr)
        {
            ...
            if (distVec1->length() > distVec2->length())
            {
                ...
                {
                    util = util
                            + 5 * (1 - ConstraintBuilder::distanceSqr(optP1, tMidFieldDefPos) / wm->field.getMaxDistanceSqr())
                                    * weightShiftP1;
                }
                else
                {
                    util = util
                            + 5 * (1 - ConstraintBuilder::distanceSqr(optP2, tMidFieldDefPos) / wm->field.getMaxDistanceSqr())
                                    * weightShiftP2;
                }
            }
            else
