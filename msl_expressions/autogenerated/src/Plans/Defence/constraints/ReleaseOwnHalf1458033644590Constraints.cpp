...
namespace alicaAutogenerated
{
    ...
    void Constraint1458033759784::getConstraint(shared_ptr<ConstraintDescriptor> c, shared_ptr<RunningPlan> rp)
    {
        ...
        c->getStaticRanges()->at(0).at(0) = -field->FieldLength / 2.0 + field->PenaltyAreaLength;
        c->getStaticRanges()->at(0).at(1) = field->FieldLength / 2.0 - field->PenaltyAreaWidth;
        ...
        if (pos != nullptr)
        {
            ...
            if (distVec1->length() > distVec2->length())
            {
                ...
                {
                    util = util
                            + 5 * (1 - ConstraintBuilder::distanceSqr(optP1, tMidFieldDefPos) / field->MaxDistanceSqr)
                                    * weightShiftP1;
                }
                else
                {
                    util = util
                            + 5 * (1 - ConstraintBuilder::distanceSqr(optP2, tMidFieldDefPos) / field->MaxDistanceSqr)
                                    * weightShiftP2;
                }
            }
            else
