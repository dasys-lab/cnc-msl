...
void OneGernericInGameBlocker::run(void *msg)
{
    ...
    nonstd::optional<geometry::CNPositionAllo> attackerPos;
    // determine the best reference point
    if (this->teamMateId != 0)
    { // take the teammate as reference point
        attackerPos = wm->robots->teammates.getTeammatePositionBuffer(teamMateId).getLastValidContent();
    }
    ...
    cout << "BEH " << this->getRunningPlan()->getPlan()->getName()
         << ": Solver found valid solution: " << (ret ? "true" : "false") << endl;
    ...
    auto driveTo = geometry::CNPointAllo(result.at(0), result.at(1));

    auto ownRelativePos = ownPos->getPoint().toEgo(attackerPos.value());
    auto relativeGoalPos = driveTo.toEgo(attackerPos.value());

    if (relativeGoalPos.x < -250 && // block only targets which are in driving direction of attacker
        ownRelativePos.length() >
            relativeGoalPos.length() +
                1000) // i'm more than 1 meter away from the blocking position, i failed, better set status to success
    {
        this->setSuccess(true);
        return;
    }
