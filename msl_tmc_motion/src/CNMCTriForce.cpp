...
//		this.logTypes = this.sc["Motion"].TryGetAllStrings(this.logTypes,"Motion","CNMC","LogType");
	...
	void CNMCTriForce::openInternal()
	{
		...
		std::string dev("/dev");
		std::size_t found = this->device.find(dev);
...
//			UpdateMotorState(null,(CNMCPacketRequestResponse)r);
	...
	void CNMCTriForce::getMotorConfig()
	{
		...
		this->mc->maxAcceleration = (*sc)["Motion"]->get<double>("Motion", "CNMC", "Controller", "MaxAcceleration", NULL);
		this->mc->maxDecceleration = (*sc)["Motion"]->get<double>("Motion", "CNMC", "Controller", "MaxDecceleration", NULL);
